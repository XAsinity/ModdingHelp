...
implements IWeightedElement {
    public static final BuilderCodec<WeatherForecast> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(WeatherForecast.class, WeatherForecast::new).append(new KeyedCodec<String>("WeatherId", Codec.STRING), (weatherForecast, s) -> {
        weatherForecast.weatherId = s;
    }, weatherForecast -> weatherForecast.weatherId).addValidator(Validators.nonNull()).addValidator(Weather.VALIDATOR_CACHE.getValidator()).add()).addField(new KeyedCodec<Double>("Weight", Codec.DOUBLE, true), (spawn, s) -> {
        spawn.weight = s;
    }, spawn -> spawn.weight)).afterDecode(WeatherForecast::processConfig)).build();
