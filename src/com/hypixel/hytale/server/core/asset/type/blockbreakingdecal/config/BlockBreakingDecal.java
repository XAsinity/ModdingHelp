...
NetworkSerializable<com.hypixel.hytale.protocol.BlockBreakingDecal> {
    ...
    public static final AssetCodec<String, BlockBreakingDecal> CODEC = ((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(BlockBreakingDecal.class, BlockBreakingDecal::new, Codec.STRING, (t, k) -> {
        t.id = k;
    }, t -> t.id, (asset, data) -> {
        asset.data = data;
    }, asset -> asset.data).append(new KeyedCodec<T[]>("StageTextures", Codec.STRING_ARRAY), (blockSet, strings) -> {
        blockSet.stageTextures = strings;
    }, blockSet -> blockSet.stageTextures).addValidator(new ArrayValidator<String>(CommonAssetValidator.TEXTURE_ITEM)).add()).build();
    ...
    public String toString() {
        return "BlockBreakingDecal{id='" + this.id + "', data=" + String.valueOf(this.data) + ", stageTextures=" + Arrays.toString(this.stageTextures) + "}";
    }
