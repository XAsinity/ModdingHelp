...
public class CollisionModuleConfig {
    public static final BuilderCodec<CollisionModuleConfig> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(CollisionModuleConfig.class, CollisionModuleConfig::new).addField(new KeyedCodec<Double>("ExtentMax", Codec.DOUBLE), (config, d) -> {
        config.extentMax = d;
    }, config -> config.extentMax)).addField(new KeyedCodec<Boolean>("DumpInvalidBlocks", Codec.BOOLEAN), (config, b) -> {
        config.dumpInvalidBlocks = b;
    }, config -> config.dumpInvalidBlocks)).addField(new KeyedCodec<Double>("MinimumThickness", Codec.DOUBLE), (config, d) -> {
        config.minimumThickness = d;
    }, config -> config.minimumThickness)).build();
