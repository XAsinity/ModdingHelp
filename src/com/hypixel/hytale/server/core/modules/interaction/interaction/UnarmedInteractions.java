...
implements JsonAssetWithMap<String, DefaultAssetMap<String, UnarmedInteractions>> {
    ...
    public static final AssetBuilderCodec<String, UnarmedInteractions> CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(UnarmedInteractions.class, UnarmedInteractions::new, Codec.STRING, (t, k) -> {
        t.id = k;
    }, t -> t.id, (t, data) -> {
        t.data = data;
    }, t -> t.data).appendInherited(new KeyedCodec("Interactions", new EnumMapCodec(InteractionType.class, RootInteraction.CHILD_ASSET_CODEC, false)), (item, v) -> {
        item.interactions = v;
    }, item -> item.interactions, (item, parent) -> {
        item.interactions = parent.interactions;
    }).addValidator(RootInteraction.VALIDATOR_CACHE.getMapValueValidator()).add()).afterDecode(o -> {
        if (o.interactions == null) {
            o.interactions = new EnumMap<InteractionType, String>(InteractionType.class);
        }
    ...
    public String toString() {
        return "UnarmedInteractions{id=" + this.id + ", interactions=" + String.valueOf(this.interactions) + "}";
    }
}
