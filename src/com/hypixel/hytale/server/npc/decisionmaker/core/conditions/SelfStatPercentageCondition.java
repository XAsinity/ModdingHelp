...
extends CurveCondition {
    public static final BuilderCodec<SelfStatPercentageCondition> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SelfStatPercentageCondition.class, SelfStatPercentageCondition::new, CurveCondition.ABSTRACT_CODEC).documentation("A curve condition that returns a utility value based on the percentage value of one of this NPC's stats.")).appendInherited(new KeyedCodec<String>("Stat", Codec.STRING), (condition, s) -> {
        condition.stat = s;
    }, condition -> condition.stat, (condition, parent) -> {
        condition.stat = parent.stat;
    }).addValidator(Validators.nonNull()).addValidator(EntityStatType.VALIDATOR_CACHE.getValidator()).documentation("The stat to check.").add()).afterDecode(condition -> {
        condition.statIndex = EntityStatType.getAssetMap().getIndex(condition.stat);
    })).build();
