...
implements IAnnotatedComponentCollection {
    ...
    public EntityFilterMany(@Nonnull List<IEntityFilter> filters) {
        ...
        for (IEntityFilter filter : this.filters = (IEntityFilter[])filters.toArray(IEntityFilter[]::new)) {
        ...
        for (int i = 0; i < this.filters.length; ++i) {
            cost = (int)((double)cost + (double)this.filters[i].cost() * (1.0 / Math.pow(2.0, i)));
        }
        this.cost = cost;
    }
