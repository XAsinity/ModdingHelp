...
public class PipeCaveNodeShape
...
implements IWorldBounds {
    ...
    public boolean shouldReplace(int seed, double x, double z, int y) {
        ...
        return (dx -= x) * dx + (dy -= (double)y) * dy + (dz -= z) * dz < r * r;
    }
    ...
    implements CaveNodeShapeEnum.CaveNodeShapeGenerator {
        ...
        public CaveNodeShape generateCaveNodeShape(@Nonnull Random random, CaveType caveType, CaveNode parentNode, @Nonnull CaveNodeType.CaveNodeChildEntry childEntry, @Nonnull Vector3d origin, float yaw, float pitch) {
            ...
            Vector3d direction = new Vector3d(TrigMathUtil.sin(pitch) * TrigMathUtil.cos(yaw), TrigMathUtil.cos(pitch), TrigMathUtil.sin(pitch) * TrigMathUtil.sin(yaw)).scale(l);
            double radius1 = this.inheritParentRadius ? CaveNodeShapeUtils.getEndRadius(parentNode, this.radius, random) : this.radius.getValue(random.nextDouble());
