...
extends SimpleBlockInteraction {
    ...
    public RefillContainerInteraction(@Nonnull WaitForDataFrom waitForDataFrom, @Nullable InteractionEffects effects, float horizontalSpeedMultiplier, float runTime, boolean cancelOnItemChange, @Nullable Map<GameMode, InteractionSettings> settings, @Nullable InteractionRules rules, @Nullable int[] tags, @Nullable InteractionCameraSettings camera, int next, int failed, boolean useLatestTarget, @Nullable int[] refillFluids) {
        ...
        this.refillFluids = refillFluids;
    }
    ...
    public RefillContainerInteraction(@Nonnull RefillContainerInteraction other) {
        ...
        this.refillFluids = other.refillFluids;
    }
    ...
    public static RefillContainerInteraction deserialize(@Nonnull ByteBuf buf, int offset) {
        int i;
        int varIntLen;
        RefillContainerInteraction obj = new RefillContainerInteraction();
        ...
        if ((nullBits & 0x20) != 0) {
            ...
            int refillFluidsCount = VarInt.peek(buf, varPos5);
            if (refillFluidsCount < 0) {
                throw ProtocolException.negativeLength("RefillFluids", refillFluidsCount);
            }
            if (refillFluidsCount > 4096000) {
                throw ProtocolException.arrayTooLong("RefillFluids", refillFluidsCount, 4096000);
            }
            varIntLen = VarInt.length(buf, varPos5);
            if ((long)(varPos5 + varIntLen) + (long)refillFluidsCount * 4L > (long)buf.readableBytes()) {
                throw ProtocolException.bufferTooSmall("RefillFluids", varPos5 + varIntLen + refillFluidsCount * 4, buf.readableBytes());
            }
            obj.refillFluids = new int[refillFluidsCount];
            for (i = 0; i < refillFluidsCount; ++i) {
                obj.refillFluids[i] = buf.getIntLE(varPos5 + varIntLen + i * 4);
            }
        }
        return obj;
    }
    ...
    public int serialize(@Nonnull ByteBuf buf) {
        ...
        buf.writeIntLE(0);
        int refillFluidsOffsetSlot = buf.writerIndex();
        buf.writeIntLE(0);
        ...
        if (this.refillFluids != null) {
            buf.setIntLE(refillFluidsOffsetSlot, buf.writerIndex() - varBlockStart);
            if (this.refillFluids.length > 4096000) {
                throw ProtocolException.arrayTooLong("RefillFluids", this.refillFluids.length, 4096000);
            }
            VarInt.write(buf, this.refillFluids.length);
            for (Object item : (Object)this.refillFluids) {
                buf.writeIntLE((int)item);
            }
        } else {
            buf.setIntLE(refillFluidsOffsetSlot, -1);
        }
    ...
    public int computeSize() {
        ...
        if (this.refillFluids != null) {
            size += VarInt.size(this.refillFluids.length) + this.refillFluids.length * 4;
        }
        return size;
    }
    ...
    public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
        ...
        if ((nullBits & 0x20) != 0) {
            int refillFluidsOffset = buffer.getIntLE(offset + 40);
            if (refillFluidsOffset < 0) {
                return ValidationResult.error("Invalid offset for RefillFluids");
            }
            pos = offset + 44 + refillFluidsOffset;
            if (pos >= buffer.writerIndex()) {
                return ValidationResult.error("Offset out of bounds for RefillFluids");
            }
            int refillFluidsCount = VarInt.peek(buffer, pos);
            if (refillFluidsCount < 0) {
                return ValidationResult.error("Invalid array count for RefillFluids");
            }
            if (refillFluidsCount > 4096000) {
                return ValidationResult.error("RefillFluids exceeds max length 4096000");
            }
            pos += VarInt.length(buffer, pos);
            if ((pos += refillFluidsCount * 4) > buffer.writerIndex()) {
                return ValidationResult.error("Buffer overflow reading RefillFluids");
            }
        }
    ...
    public RefillContainerInteraction clone() {
        RefillContainerInteraction copy = new RefillContainerInteraction();
        ...
        copy.refillFluids = this.refillFluids != null ? Arrays.copyOf(this.refillFluids, this.refillFluids.length) : null;
        return copy;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof RefillContainerInteraction)) {
            return false;
        }
        RefillContainerInteraction other = (RefillContainerInteraction)obj;
    ...
    public int hashCode() {
        ...
        result = 31 * result + Arrays.hashCode(this.refillFluids);
        return result;
    }
}
