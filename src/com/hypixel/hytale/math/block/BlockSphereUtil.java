...
public class BlockSphereUtil {
    ...
    public static <T> void forEachBlock(int originX, int originY, int originZ, int radius, @Nullable T t, @Nonnull TriIntObjPredicate<T> consumer) {
        BlockSphereUtil.forEachBlock(originX, originY, originZ, radius, radius, radius, t, consumer);
    }
        ...
        for (int x = 0; x <= radiusX; ++x) {
            ...
            for (int y = 0; y <= maxY; ++y) {
                ...
                for (int z = 0; z <= maxZ; ++z) {
                    if (BlockSphereUtil.test(originX, originY, originZ, x, y, z, t, consumer)) continue;
                    return false;
                }
            }
        }
    ...
    public static <T> void forEachBlock(int originX, int originY, int originZ, int radius, int thickness, @Nullable T t, @Nonnull TriIntObjPredicate<T> consumer) {
        BlockSphereUtil.forEachBlock(originX, originY, originZ, radius, radius, radius, thickness, t, consumer);
    }
    ...
    public static <T> boolean forEachBlock(int originX, int originY, int originZ, int radiusX, int radiusY, int radiusZ, int thickness, @Nullable T t, @Nonnull TriIntObjPredicate<T> consumer) {
        if (thickness < 1) {
            return BlockSphereUtil.forEachBlock(originX, originY, originZ, radiusX, radiusY, radiusZ, t, consumer);
        }
        if (radiusX <= 0) {
            throw new IllegalArgumentException(String.valueOf(radiusX));
        }
        ...
        while (y <= radiusY) {
            ...
            while (x <= maxX) {
                ...
                while (z <= maxZ) {
                    ...
                    if (!(innerQx + innerQy + innerQz < 1.0f && outerQx + outerQy + (outerQz = (float)(z1 * z1) * invRadiusZ2) < 1.0f || BlockSphereUtil.test(originX, originY, originZ, x, y, z, t, consumer))) {
                        return false;
                    }
                    ++z;
                    ++z1;
