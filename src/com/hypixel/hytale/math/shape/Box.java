...
public class Box
implements Shape {
    public static final Codec<Box> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(Box.class, Box::new).append(new KeyedCodec<Vector3d>("Min", Vector3d.CODEC), (box, v) -> box.min.assign((Vector3d)v), box -> box.min).add()).append(new KeyedCodec<Vector3d>("Max", Vector3d.CODEC), (box, v) -> box.max.assign((Vector3d)v), box -> box.max).add()).validator((box, results) -> {
        if (box.width() <= 0.0) {
    ...
    public double dimension(@Nonnull Axis axis) {
        return switch (axis) {
            ...
            case Axis.X -> this.width();
            case Axis.Y -> this.height();
            case Axis.Z -> this.depth();
        };
    }
        ...
        for (int _x = minX; _x <= maxX; ++_x) {
            for (int _y = minY; _y <= maxY; ++_y) {
                for (int _z = minZ; _z <= maxZ; ++_z) {
                    if (consumer.test(_x, _y, _z)) continue;
                    return false;
                }
            }
        }
    ...
    public <T> boolean forEachBlock(double x, double y, double z, double epsilon, T t, @Nonnull TriIntObjPredicate<T> consumer) {
        ...
        for (int _x = minX; _x <= maxX; ++_x) {
            for (int _y = minY; _y <= maxY; ++_y) {
                for (int _z = minZ; _z <= maxZ; ++_z) {
                    if (consumer.test(_x, _y, _z, t)) continue;
                    return false;
                }
            }
        }
    ...
    public boolean intersectsLine(@Nonnull Vector3d start, @Nonnull Vector3d end) {
        ...
        if (Math.abs(direction.x) < 1.0E-10) {
            ...
            t1 = (this.min.x - start.x) / direction.x;
            t2 = (this.max.x - start.x) / direction.x;
            if (t1 > t2) {
                temp = t1;
                t1 = t2;
                t2 = temp;
        ...
        if (Math.abs(direction.y) < 1.0E-10) {
            ...
            t1 = (this.min.y - start.y) / direction.y;
            t2 = (this.max.y - start.y) / direction.y;
            if (t1 > t2) {
                temp = t1;
                t1 = t2;
                t2 = temp;
        ...
        t1 = (this.min.z - start.z) / direction.z;
        t2 = (this.max.z - start.z) / direction.z;
        if (t1 > t2) {
            temp = t1;
            t1 = t2;
            t2 = temp;
        }
        return !((tmin = Math.max(tmin, t1)) > (tmax = Math.min(tmax, t2)));
    }
