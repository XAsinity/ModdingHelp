...
implements JsonAssetWithMap<String, DefaultAssetMap<String, ReputationGroup>> {
    @Nonnull
    public static final AssetBuilderCodec<String, ReputationGroup> CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(ReputationGroup.class, ReputationGroup::new, Codec.STRING, (t, k) -> {
        t.id = k;
    }, t -> t.id, (t, data) -> {
        t.data = data;
    }, t -> t.data).addField(new KeyedCodec<T[]>("NPCGroups", Codec.STRING_ARRAY), (reputationRank, s) -> {
        reputationRank.npcGroups = s;
    }, reputationRank -> reputationRank.npcGroups)).addField(new KeyedCodec<Integer>("InitialReputationValue", Codec.INTEGER), (reputationRank, s) -> {
        reputationRank.initialReputationValue = s;
    }, reputationRank -> reputationRank.initialReputationValue)).build();
    ...
    public static DefaultAssetMap<String, ReputationGroup> getAssetMap() {
        return ReputationGroup.getAssetStore().getAssetMap();
    }
