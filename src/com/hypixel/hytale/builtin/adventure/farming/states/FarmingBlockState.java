...
implements Component<ChunkStore> {
    ...
    }, state -> state.stageStart).add()).append(new KeyedCodec<Integer>("CurrentFarmingStageIndex", Codec.INTEGER), (baseFarmingBlockState, integer) -> {
        baseFarmingBlockState.currentFarmingStageIndex = integer;
    }, baseFarmingBlockState -> baseFarmingBlockState.currentFarmingStageIndex).add()).append(new KeyedCodec<String>("CurrentFarmingStageSetName", Codec.STRING), (farmingBlockState, s) -> {
        farmingBlockState.currentFarmingStageSetName = s;
    }, farmingBlockState -> farmingBlockState.currentFarmingStageSetName).add()).append(new KeyedCodec<Float>("SpreadRate", Codec.FLOAT), (blockState, aFloat) -> {
        blockState.spreadRate = aFloat.floatValue();
    }, blockState -> Float.valueOf(blockState.spreadRate)).add()).build();
