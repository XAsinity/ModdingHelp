...
extends AbstractWorldCommand {
    ...
    protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
        world.getWorldPathConfig().getPaths().values().stream().sorted(Comparator.comparing(WorldPath::getName)).map(WorldPath::getName).map(Message::raw).forEach(context::sendMessage);
    }
}
