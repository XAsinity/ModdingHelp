...
public class BlockPhysicsSystems {
            ...
            for (int ix = -1; ix < 2; ++ix) {
                ...
                for (int iz = -1; iz < 2; ++iz) {
                    ...
                    for (int iy = -1; iy < 2; ++iy) {
                        ...
                        int n = support = physics != null ? physics.get(wx, wy, wz) : 0;
                        if (support > maxSupportDistance || (blockChunk = this.getBlockSection(ChunkUtil.chunkCoordinate(wx), ChunkUtil.chunkCoordinate(wy), ChunkUtil.chunkCoordinate(wz))) == null) continue;
    ...
    implements DisableProcessingAssert {
        private static final Query<ChunkStore> QUERY = Query.and(ChunkSection.getComponentType(), BlockSection.getComponentType(), BlockPhysics.getComponentType(), FluidSection.getComponentType());
        ...
        public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
            ...
            try {
                ...
                CachedAccessor accessor = CachedAccessor.of(commandBuffer, blockSection, blockPhysics, fluidSection, section.getX(), section.getY(), section.getZ(), 14);
                blockSection.forEachTicking(worldChunk, accessor, section.getY(), (wc, accessor1, localX, localY, localZ, blockId) -> {
                    ...
                    return switch (BlockPhysicsUtil.applyBlockPhysics(entityStore, wc.getReference(), accessor, accessor1.selfBlockSection, accessor1.selfPhysics, accessor1.selfFluidSection, blockX, blockY, blockZ, blockType, rotation, filler)) {
                        ...
                        case BlockPhysicsUtil.Result.WAITING_CHUNK -> BlockTickStrategy.WAIT_FOR_ADJACENT_CHUNK_LOAD;
                        case BlockPhysicsUtil.Result.VALID -> BlockTickStrategy.IGNORED;
                        case BlockPhysicsUtil.Result.INVALID -> BlockTickStrategy.SLEEP;
                    };
                });
            }
            catch (Exception t) {
                ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause(t)).log("Failed to tick chunk: %s", section);
            }
        }
    }
}
