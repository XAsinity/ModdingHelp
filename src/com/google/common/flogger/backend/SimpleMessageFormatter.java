...
public final class SimpleMessageFormatter {
    ...
    public static StringBuilder appendFormatted(LogData logData, MetadataProcessor metadata, MetadataHandler<MetadataKey.KeyValueHandler> metadataHandler, StringBuilder buffer) {
        ...
        return SimpleMessageFormatter.appendContext(metadata, metadataHandler, buffer);
    }
    ...
    private static String format(LogData logData, MetadataProcessor metadata) {
        return SimpleMessageFormatter.mustBeFormatted(logData, metadata, DEFAULT_KEYS_TO_IGNORE) ? SimpleMessageFormatter.appendFormatted(logData, metadata, DEFAULT_HANDLER, new StringBuilder()).toString() : SimpleMessageFormatter.getLiteralLogMessage(logData);
    }
    ...
    public static void format(LogData logData, SimpleLogHandler receiver) {
        ...
        receiver.handleFormattedLogMessage(logData.getLevel(), SimpleMessageFormatter.format(logData, metadata), metadata.getSingleValue(LogContext.Key.LOG_CAUSE));
    }
    ...
    static void format(LogData logData, SimpleLogHandler receiver, Option option) {
        switch (option) {
            case WITH_LOG_SITE: {
                ...
                String message = SimpleMessageFormatter.appendFormatted(logData, MetadataProcessor.forScopeAndLogSite(Metadata.empty(), logData.getMetadata()), DEFAULT_HANDLER, buffer).toString();
