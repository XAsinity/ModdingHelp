...
public final class JwtEcdsaProtoSerialization {
    ...
    private static JwtEcdsaKeyFormat serializeToJwtEcdsaKeyFormat(JwtEcdsaParameters parameters) throws GeneralSecurityException {
        ...
        return JwtEcdsaKeyFormat.newBuilder().setVersion(0).setAlgorithm(JwtEcdsaProtoSerialization.toProtoAlgorithm(parameters.getAlgorithm())).build();
    }
    ...
    private static ProtoParametersSerialization serializeParameters(JwtEcdsaParameters parameters) throws GeneralSecurityException {
        ...
        return ProtoParametersSerialization.create(KeyTemplate.newBuilder().setTypeUrl(TYPE_URL).setValue(JwtEcdsaProtoSerialization.serializeToJwtEcdsaKeyFormat(parameters).toByteString()).setOutputPrefixType(outputPrefixType).build());
    }
    ...
    private static JwtEcdsaParameters parseParameters(ProtoParametersSerialization serialization) throws GeneralSecurityException {
        ...
        return JwtEcdsaParameters.builder().setAlgorithm(JwtEcdsaProtoSerialization.toAlgorithm(format.getAlgorithm())).setKidStrategy(kidStrategy).build();
    }
    ...
    private static com.google.crypto.tink.proto.JwtEcdsaPublicKey serializePublicKey(JwtEcdsaPublicKey key) throws GeneralSecurityException {
        int encLength = JwtEcdsaProtoSerialization.getEncodingLength(key.getParameters().getAlgorithm());
        ECPoint publicPoint = key.getPublicPoint();
        JwtEcdsaPublicKey.Builder builder = com.google.crypto.tink.proto.JwtEcdsaPublicKey.newBuilder().setVersion(0).setAlgorithm(JwtEcdsaProtoSerialization.toProtoAlgorithm(key.getParameters().getAlgorithm())).setX(ByteString.copyFrom(BigIntegerEncoding.toBigEndianBytesOfFixedLength(publicPoint.getAffineX(), encLength))).setY(ByteString.copyFrom(BigIntegerEncoding.toBigEndianBytesOfFixedLength(publicPoint.getAffineY(), encLength)));
        if (key.getParameters().getKidStrategy().equals(JwtEcdsaParameters.KidStrategy.CUSTOM)) {
            builder.setCustomKid(JwtEcdsaPublicKey.CustomKid.newBuilder().setValue(key.getKid().get()).build());
        }
    ...
    private static ProtoKeySerialization serializePublicKey(JwtEcdsaPublicKey key, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        return ProtoKeySerialization.create(PUBLIC_TYPE_URL, JwtEcdsaProtoSerialization.serializePublicKey(key).toByteString(), KeyData.KeyMaterialType.ASYMMETRIC_PUBLIC, JwtEcdsaProtoSerialization.toProtoOutputPrefixType(key.getParameters()), key.getIdRequirementOrNull());
    }
    ...
    private static JwtEcdsaPublicKey parsePublicKeyFromProto(com.google.crypto.tink.proto.JwtEcdsaPublicKey protoKey, OutputPrefixType outputPrefixType, @Nullable Integer idRequirement) throws GeneralSecurityException {
        ...
        parametersBuilder.setAlgorithm(JwtEcdsaProtoSerialization.toAlgorithm(protoKey.getAlgorithm()));
    ...
    private static JwtEcdsaPublicKey parsePublicKey(ProtoKeySerialization serialization, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        ...
        try {
            ...
            return JwtEcdsaProtoSerialization.parsePublicKeyFromProto(protoKey, serialization.getOutputPrefixType(), serialization.getIdRequirementOrNull());
        }
        catch (InvalidProtocolBufferException e) {
    ...
    private static com.google.crypto.tink.proto.JwtEcdsaPrivateKey serializePrivateKeyToProto(JwtEcdsaPrivateKey key, SecretKeyAccess access) throws GeneralSecurityException {
        int encLength = JwtEcdsaProtoSerialization.getEncodingLength(key.getParameters().getAlgorithm());
        return com.google.crypto.tink.proto.JwtEcdsaPrivateKey.newBuilder().setPublicKey(JwtEcdsaProtoSerialization.serializePublicKey(key.getPublicKey())).setKeyValue(ByteString.copyFrom(BigIntegerEncoding.toBigEndianBytesOfFixedLength(key.getPrivateValue().getBigInteger(access), encLength))).build();
    }
    ...
    private static ProtoKeySerialization serializePrivateKey(JwtEcdsaPrivateKey key, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        return ProtoKeySerialization.create(TYPE_URL, JwtEcdsaProtoSerialization.serializePrivateKeyToProto(key, SecretKeyAccess.requireAccess(access)).toByteString(), KeyData.KeyMaterialType.ASYMMETRIC_PRIVATE, JwtEcdsaProtoSerialization.toProtoOutputPrefixType(key.getParameters()), key.getIdRequirementOrNull());
    }
    ...
    private static JwtEcdsaPrivateKey parsePrivateKey(ProtoKeySerialization serialization, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        ...
        try {
            ...
            JwtEcdsaPublicKey publicKey = JwtEcdsaProtoSerialization.parsePublicKeyFromProto(protoKey.getPublicKey(), serialization.getOutputPrefixType(), serialization.getIdRequirementOrNull());
    ...
    public static void register() throws GeneralSecurityException {
        JwtEcdsaProtoSerialization.register(MutableSerializationRegistry.globalInstance());
    }
