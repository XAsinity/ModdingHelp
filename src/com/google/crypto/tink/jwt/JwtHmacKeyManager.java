...
public final class JwtHmacKeyManager {
    ...
    private static final KeyCreator<JwtHmacParameters> KEY_CREATOR = JwtHmacKeyManager::createKey;
    ...
    private static JwtMac createFullJwtHmac(JwtHmacKey key) throws GeneralSecurityException {
        JwtHmacKeyManager.validate(key.getParameters());
        HmacKey hmacKey = HmacKey.builder().setParameters(HmacParameters.builder().setKeySizeBytes(key.getParameters().getKeySizeBytes()).setHashType(JwtHmacKeyManager.getHmacHashType(key.getParameters().getAlgorithm())).setTagSizeBytes(JwtHmacKeyManager.getTagLength(key.getParameters().getAlgorithm())).build()).setKeyBytes(key.getKeyBytes()).build();
    ...
    private static JwtHmacKey createKey(JwtHmacParameters parameters, @Nullable Integer idRequirement) throws GeneralSecurityException {
        JwtHmacKeyManager.validate(parameters);
