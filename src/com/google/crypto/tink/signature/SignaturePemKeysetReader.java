...
implements KeysetReader {
    ...
    private static KeyData convertRsaPublicKey(PemKeyType pemKeyType, RSAPublicKey key) throws IOException {
        if (pemKeyType.algorithm.equals("RSASSA-PKCS1-v1_5")) {
            RsaSsaPkcs1Params params = RsaSsaPkcs1Params.newBuilder().setHashType(SignaturePemKeysetReader.getHashType(pemKeyType)).build();
            RsaSsaPkcs1PublicKey pkcs1PubKey = RsaSsaPkcs1PublicKey.newBuilder().setVersion(0).setParams(params).setE(SigUtil.toUnsignedIntByteString(key.getPublicExponent())).setN(SigUtil.toUnsignedIntByteString(key.getModulus())).build();
        ...
        if (pemKeyType.algorithm.equals("RSASSA-PSS")) {
            RsaSsaPssParams params = RsaSsaPssParams.newBuilder().setSigHash(SignaturePemKeysetReader.getHashType(pemKeyType)).setMgf1Hash(SignaturePemKeysetReader.getHashType(pemKeyType)).setSaltLength(SignaturePemKeysetReader.getDigestSizeInBytes(pemKeyType)).build();
            RsaSsaPssPublicKey pssPubKey = RsaSsaPssPublicKey.newBuilder().setVersion(0).setParams(params).setE(SigUtil.toUnsignedIntByteString(key.getPublicExponent())).setN(SigUtil.toUnsignedIntByteString(key.getModulus())).build();
    ...
    private static KeyData convertEcPublicKey(PemKeyType pemKeyType, ECPublicKey key) throws IOException {
        if (pemKeyType.algorithm.equals("ECDSA")) {
            EcdsaParams params = EcdsaParams.newBuilder().setHashType(SignaturePemKeysetReader.getHashType(pemKeyType)).setCurve(SignaturePemKeysetReader.getCurveType(pemKeyType)).setEncoding(EcdsaSignatureEncoding.DER).build();
            EcdsaPublicKey ecdsaPubKey = EcdsaPublicKey.newBuilder().setVersion(0).setParams(params).setX(SigUtil.toUnsignedIntByteString(key.getW().getAffineX())).setY(SigUtil.toUnsignedIntByteString(key.getW().getAffineY())).build();
