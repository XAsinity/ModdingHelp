...
implements PublicKeyVerify {
    ...
    public static PublicKeyVerify create(RsaSsaPkcs1PublicKey key) throws GeneralSecurityException {
        Provider conscrypt = RsaSsaPkcs1VerifyConscrypt.conscryptProviderOrNull();
        if (conscrypt == null) {
            throw new NoSuchProviderException("RSA-PKCS1.5 using Conscrypt is not supported.");
        }
        return RsaSsaPkcs1VerifyConscrypt.createWithProvider(key, conscrypt);
    }
    ...
    public static PublicKeyVerify createWithProvider(RsaSsaPkcs1PublicKey key, Provider conscrypt) throws GeneralSecurityException {
        ...
        return new RsaSsaPkcs1VerifyConscrypt(publicKey, key.getParameters().getHashType(), key.getOutputPrefix().toByteArray(), key.getParameters().getVariant().equals(RsaSsaPkcs1Parameters.Variant.LEGACY) ? legacyMessageSuffix : EMPTY, conscrypt);
    }
