...
implements PublicKeyVerify {
    ...
    public static PublicKeyVerify create(EcdsaPublicKey key) throws GeneralSecurityException {
        ...
        return EcdsaVerifyJce.createWithProviderOrNull(key, provider);
    }
    ...
    public static PublicKeyVerify createWithProviderOrNull(EcdsaPublicKey key, @Nullable Provider provider) throws GeneralSecurityException {
        ...
        KeyFactory keyFactory = provider != null ? KeyFactory.getInstance("EC", provider) : EngineFactory.KEY_FACTORY.getInstance("EC");
        ECPublicKey publicKey = (ECPublicKey)keyFactory.generatePublic(spec);
        return new EcdsaVerifyJce(publicKey, HASH_TYPE_CONVERTER.toProtoEnum(key.getParameters().getHashType()), ENCODING_CONVERTER.toProtoEnum(key.getParameters().getSignatureEncoding()), key.getOutputPrefix().toByteArray(), key.getParameters().getVariant().equals(EcdsaParameters.Variant.LEGACY) ? legacyMessageSuffix : EMPTY, provider);
    }
