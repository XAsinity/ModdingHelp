...
public final class RsaSsaPssProtoSerialization {
    ...
    private static RsaSsaPssParams getProtoParams(RsaSsaPssParameters parameters) throws GeneralSecurityException {
        return RsaSsaPssParams.newBuilder().setSigHash(HASH_TYPE_CONVERTER.toProtoEnum(parameters.getSigHashType())).setMgf1Hash(HASH_TYPE_CONVERTER.toProtoEnum(parameters.getMgf1HashType())).setSaltLength(parameters.getSaltLengthBytes()).build();
    }
    ...
    private static com.google.crypto.tink.proto.RsaSsaPssPublicKey getProtoPublicKey(RsaSsaPssPublicKey key) throws GeneralSecurityException {
        return com.google.crypto.tink.proto.RsaSsaPssPublicKey.newBuilder().setParams(RsaSsaPssProtoSerialization.getProtoParams(key.getParameters())).setN(RsaSsaPssProtoSerialization.encodeBigInteger(key.getModulus())).setE(RsaSsaPssProtoSerialization.encodeBigInteger(key.getParameters().getPublicExponent())).setVersion(0).build();
    }
    ...
    private static ProtoParametersSerialization serializeParameters(RsaSsaPssParameters parameters) throws GeneralSecurityException {
        return ProtoParametersSerialization.create(KeyTemplate.newBuilder().setTypeUrl(PRIVATE_TYPE_URL).setValue(RsaSsaPssKeyFormat.newBuilder().setParams(RsaSsaPssProtoSerialization.getProtoParams(parameters)).setModulusSizeInBits(parameters.getModulusSizeBits()).setPublicExponent(RsaSsaPssProtoSerialization.encodeBigInteger(parameters.getPublicExponent())).build().toByteString()).setOutputPrefixType(VARIANT_CONVERTER.toProtoEnum(parameters.getVariant())).build());
    }
    ...
    private static ProtoKeySerialization serializePublicKey(RsaSsaPssPublicKey key, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        return ProtoKeySerialization.create(PUBLIC_TYPE_URL, RsaSsaPssProtoSerialization.getProtoPublicKey(key).toByteString(), KeyData.KeyMaterialType.ASYMMETRIC_PUBLIC, VARIANT_CONVERTER.toProtoEnum(key.getParameters().getVariant()), key.getIdRequirementOrNull());
    }

    private static ByteString encodeSecretBigInteger(SecretBigInteger i, SecretKeyAccess access) {
        return RsaSsaPssProtoSerialization.encodeBigInteger(i.getBigInteger(access));
    }
    ...
    private static ProtoKeySerialization serializePrivateKey(RsaSsaPssPrivateKey key, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        ...
        com.google.crypto.tink.proto.RsaSsaPssPrivateKey protoPrivateKey = com.google.crypto.tink.proto.RsaSsaPssPrivateKey.newBuilder().setVersion(0).setPublicKey(RsaSsaPssProtoSerialization.getProtoPublicKey(key.getPublicKey())).setD(RsaSsaPssProtoSerialization.encodeSecretBigInteger(key.getPrivateExponent(), a)).setP(RsaSsaPssProtoSerialization.encodeSecretBigInteger(key.getPrimeP(), a)).setQ(RsaSsaPssProtoSerialization.encodeSecretBigInteger(key.getPrimeQ(), a)).setDp(RsaSsaPssProtoSerialization.encodeSecretBigInteger(key.getPrimeExponentP(), a)).setDq(RsaSsaPssProtoSerialization.encodeSecretBigInteger(key.getPrimeExponentQ(), a)).setCrt(RsaSsaPssProtoSerialization.encodeSecretBigInteger(key.getCrtCoefficient(), a)).build();
    ...
    private static RsaSsaPssParameters parseParameters(ProtoParametersSerialization serialization) throws GeneralSecurityException {
        ...
        return RsaSsaPssParameters.builder().setSigHashType(HASH_TYPE_CONVERTER.fromProtoEnum(format.getParams().getSigHash())).setMgf1HashType(HASH_TYPE_CONVERTER.fromProtoEnum(format.getParams().getMgf1Hash())).setPublicExponent(RsaSsaPssProtoSerialization.decodeBigInteger(format.getPublicExponent())).setModulusSizeBits(format.getModulusSizeInBits()).setSaltLengthBytes(format.getParams().getSaltLength()).setVariant(VARIANT_CONVERTER.fromProtoEnum(serialization.getKeyTemplate().getOutputPrefixType())).build();
    }
    ...
    private static RsaSsaPssPublicKey parsePublicKey(ProtoKeySerialization serialization, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        ...
        try {
            com.google.crypto.tink.proto.RsaSsaPssPublicKey protoKey = com.google.crypto.tink.proto.RsaSsaPssPublicKey.parseFrom(serialization.getValue(), ExtensionRegistryLite.getEmptyRegistry());
            if (protoKey.getVersion() != 0) {
                throw new GeneralSecurityException("Only version 0 keys are accepted");
            }
            BigInteger modulus = RsaSsaPssProtoSerialization.decodeBigInteger(protoKey.getN());
            int modulusSizeInBits = modulus.bitLength();
            RsaSsaPssParameters parameters = RsaSsaPssParameters.builder().setSigHashType(HASH_TYPE_CONVERTER.fromProtoEnum(protoKey.getParams().getSigHash())).setMgf1HashType(HASH_TYPE_CONVERTER.fromProtoEnum(protoKey.getParams().getMgf1Hash())).setPublicExponent(RsaSsaPssProtoSerialization.decodeBigInteger(protoKey.getE())).setModulusSizeBits(modulusSizeInBits).setSaltLengthBytes(protoKey.getParams().getSaltLength()).setVariant(VARIANT_CONVERTER.fromProtoEnum(serialization.getOutputPrefixType())).build();
            return RsaSsaPssPublicKey.builder().setParameters(parameters).setModulus(modulus).setIdRequirement(serialization.getIdRequirementOrNull()).build();
        }
    ...
    private static RsaSsaPssPrivateKey parsePrivateKey(ProtoKeySerialization serialization, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        ...
        try {
            ...
            BigInteger modulus = RsaSsaPssProtoSerialization.decodeBigInteger(protoPublicKey.getN());
            int modulusSizeInBits = modulus.bitLength();
            BigInteger publicExponent = RsaSsaPssProtoSerialization.decodeBigInteger(protoPublicKey.getE());
            RsaSsaPssParameters parameters = RsaSsaPssParameters.builder().setSigHashType(HASH_TYPE_CONVERTER.fromProtoEnum(protoPublicKey.getParams().getSigHash())).setMgf1HashType(HASH_TYPE_CONVERTER.fromProtoEnum(protoPublicKey.getParams().getMgf1Hash())).setPublicExponent(publicExponent).setModulusSizeBits(modulusSizeInBits).setSaltLengthBytes(protoPublicKey.getParams().getSaltLength()).setVariant(VARIANT_CONVERTER.fromProtoEnum(serialization.getOutputPrefixType())).build();
            RsaSsaPssPublicKey publicKey = RsaSsaPssPublicKey.builder().setParameters(parameters).setModulus(modulus).setIdRequirement(serialization.getIdRequirementOrNull()).build();
            SecretKeyAccess a = SecretKeyAccess.requireAccess(access);
            return RsaSsaPssPrivateKey.builder().setPublicKey(publicKey).setPrimes(RsaSsaPssProtoSerialization.decodeSecretBigInteger(protoKey.getP(), a), RsaSsaPssProtoSerialization.decodeSecretBigInteger(protoKey.getQ(), a)).setPrivateExponent(RsaSsaPssProtoSerialization.decodeSecretBigInteger(protoKey.getD(), a)).setPrimeExponents(RsaSsaPssProtoSerialization.decodeSecretBigInteger(protoKey.getDp(), a), RsaSsaPssProtoSerialization.decodeSecretBigInteger(protoKey.getDq(), a)).setCrtCoefficient(RsaSsaPssProtoSerialization.decodeSecretBigInteger(protoKey.getCrt(), a)).build();
        }
        catch (InvalidProtocolBufferException | IllegalArgumentException e) {
    ...
    public static void register() throws GeneralSecurityException {
        RsaSsaPssProtoSerialization.register(MutableSerializationRegistry.globalInstance());
    }
