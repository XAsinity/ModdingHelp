...
implements KeyManager<Void> {
    ...
    public final KeyData newKeyData(ByteString serializedKeyFormat) throws GeneralSecurityException {
        ...
        ProtoParametersSerialization parametersSerialization = ProtoParametersSerialization.checkedCreate(KeyTemplate.newBuilder().setTypeUrl(TYPE_URL).setValue(serializedKeyFormat).setOutputPrefixType(outputPrefixType).build());
