...
implements PrimitiveWrapper<Aead, Aead> {
    ...
    public Aead wrap(KeysetHandleInterface keysetHandle, MonitoringAnnotations annotations, PrimitiveWrapper.PrimitiveFactory<Aead> factory) throws GeneralSecurityException {
        ...
        for (int i = 0; i < keysetHandle.size(); ++i) {
            ...
            builder.put(AeadWrapper.getOutputPrefix(entry.getKey()), new AeadWithId(factory.create(entry), entry.getId()));
        }
        ...
        return new WrappedAead(new AeadWithId(factory.create(keysetHandle.getPrimary()), keysetHandle.getPrimary().getId()), builder.build(), encLogger, decLogger);
    }
