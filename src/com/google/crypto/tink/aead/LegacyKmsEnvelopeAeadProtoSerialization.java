...
public final class LegacyKmsEnvelopeAeadProtoSerialization {
    ...
    private static ProtoParametersSerialization serializeParameters(LegacyKmsEnvelopeAeadParameters parameters) throws GeneralSecurityException {
        return ProtoParametersSerialization.create(KeyTemplate.newBuilder().setTypeUrl(TYPE_URL).setValue(LegacyKmsEnvelopeAeadProtoSerialization.serializeParametersToKmsEnvelopeAeadKeyFormat(parameters).toByteString()).setOutputPrefixType(LegacyKmsEnvelopeAeadProtoSerialization.toProtoOutputPrefixType(parameters.getVariant())).build());
    }
    ...
    private static ProtoKeySerialization serializeKey(LegacyKmsEnvelopeAeadKey key, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        return ProtoKeySerialization.create(TYPE_URL, KmsEnvelopeAeadKey.newBuilder().setParams(LegacyKmsEnvelopeAeadProtoSerialization.serializeParametersToKmsEnvelopeAeadKeyFormat(key.getParameters())).build().toByteString(), KeyData.KeyMaterialType.REMOTE, LegacyKmsEnvelopeAeadProtoSerialization.toProtoOutputPrefixType(key.getParameters().getVariant()), key.getIdRequirementOrNull());
    }
    ...
    private static LegacyKmsEnvelopeAeadParameters parseParameters(ProtoParametersSerialization serialization) throws GeneralSecurityException {
        ...
        return LegacyKmsEnvelopeAeadProtoSerialization.parseParameters(format, serialization.getKeyTemplate().getOutputPrefixType());
    }
    ...
    private static LegacyKmsEnvelopeAeadParameters parseParameters(KmsEnvelopeAeadKeyFormat format, OutputPrefixType outputPrefixType) throws GeneralSecurityException {
        ...
        return LegacyKmsEnvelopeAeadParameters.builder().setVariant(LegacyKmsEnvelopeAeadProtoSerialization.toVariant(outputPrefixType)).setKekUri(format.getKekUri()).setDekParametersForNewKeys((AeadParameters)aeadParameters).setDekParsingStrategy(strategy).build();
    }
    ...
    private static LegacyKmsEnvelopeAeadKey parseKey(ProtoKeySerialization serialization, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        ...
        try {
            ...
            LegacyKmsEnvelopeAeadParameters parameters = LegacyKmsEnvelopeAeadProtoSerialization.parseParameters(protoKey.getParams(), serialization.getOutputPrefixType());
    ...
    public static void register() throws GeneralSecurityException {
        LegacyKmsEnvelopeAeadProtoSerialization.register(MutableSerializationRegistry.globalInstance());
    }
