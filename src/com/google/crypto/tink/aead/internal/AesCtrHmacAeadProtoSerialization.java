...
public final class AesCtrHmacAeadProtoSerialization {
    ...
    private static final ParametersSerializer<AesCtrHmacAeadParameters, ProtoParametersSerialization> PARAMETERS_SERIALIZER = ParametersSerializer.create(AesCtrHmacAeadProtoSerialization::serializeParameters, AesCtrHmacAeadParameters.class, ProtoParametersSerialization.class);
    private static final ParametersParser<ProtoParametersSerialization> PARAMETERS_PARSER = ParametersParser.create(AesCtrHmacAeadProtoSerialization::parseParameters, TYPE_URL_BYTES, ProtoParametersSerialization.class);
    private static final KeySerializer<AesCtrHmacAeadKey, ProtoKeySerialization> KEY_SERIALIZER = KeySerializer.create(AesCtrHmacAeadProtoSerialization::serializeKey, AesCtrHmacAeadKey.class, ProtoKeySerialization.class);
    private static final KeyParser<ProtoKeySerialization> KEY_PARSER = KeyParser.create(AesCtrHmacAeadProtoSerialization::parseKey, TYPE_URL_BYTES, ProtoKeySerialization.class);
    ...
    private static HmacParams getHmacProtoParams(AesCtrHmacAeadParameters parameters) throws GeneralSecurityException {
        return HmacParams.newBuilder().setTagSize(parameters.getTagSizeBytes()).setHash(AesCtrHmacAeadProtoSerialization.toProtoHashType(parameters.getHashType())).build();
    }
    ...
    private static AesCtrHmacAeadParameters parseParameters(ProtoParametersSerialization serialization) throws GeneralSecurityException {
        ...
        return AesCtrHmacAeadParameters.builder().setAesKeySizeBytes(format.getAesCtrKeyFormat().getKeySize()).setHmacKeySizeBytes(format.getHmacKeyFormat().getKeySize()).setIvSizeBytes(format.getAesCtrKeyFormat().getParams().getIvSize()).setTagSizeBytes(format.getHmacKeyFormat().getParams().getTagSize()).setHashType(AesCtrHmacAeadProtoSerialization.toHashType(format.getHmacKeyFormat().getParams().getHash())).setVariant(AesCtrHmacAeadProtoSerialization.toVariant(serialization.getKeyTemplate().getOutputPrefixType())).build();
    }
    ...
    private static AesCtrHmacAeadKey parseKey(ProtoKeySerialization serialization, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        ...
        try {
            ...
            AesCtrHmacAeadParameters parameters = AesCtrHmacAeadParameters.builder().setAesKeySizeBytes(protoKey.getAesCtrKey().getKeyValue().size()).setHmacKeySizeBytes(protoKey.getHmacKey().getKeyValue().size()).setIvSizeBytes(protoKey.getAesCtrKey().getParams().getIvSize()).setTagSizeBytes(protoKey.getHmacKey().getParams().getTagSize()).setHashType(AesCtrHmacAeadProtoSerialization.toHashType(protoKey.getHmacKey().getParams().getHash())).setVariant(AesCtrHmacAeadProtoSerialization.toVariant(serialization.getOutputPrefixType())).build();
    ...
    public static void register() throws GeneralSecurityException {
        AesCtrHmacAeadProtoSerialization.register(MutableSerializationRegistry.globalInstance());
    }
