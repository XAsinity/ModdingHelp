...
public final class HmacPrfProtoSerialization {
    ...
    private static final ParametersSerializer<HmacPrfParameters, ProtoParametersSerialization> PARAMETERS_SERIALIZER = ParametersSerializer.create(HmacPrfProtoSerialization::serializeParameters, HmacPrfParameters.class, ProtoParametersSerialization.class);
    private static final ParametersParser<ProtoParametersSerialization> PARAMETERS_PARSER = ParametersParser.create(HmacPrfProtoSerialization::parseParameters, TYPE_URL_BYTES, ProtoParametersSerialization.class);
    private static final KeySerializer<HmacPrfKey, ProtoKeySerialization> KEY_SERIALIZER = KeySerializer.create(HmacPrfProtoSerialization::serializeKey, HmacPrfKey.class, ProtoKeySerialization.class);
    private static final KeyParser<ProtoKeySerialization> KEY_PARSER = KeyParser.create(HmacPrfProtoSerialization::parseKey, TYPE_URL_BYTES, ProtoKeySerialization.class);
    ...
    private static HmacPrfParams getProtoParams(HmacPrfParameters parameters) throws GeneralSecurityException {
        return HmacPrfParams.newBuilder().setHash(HmacPrfProtoSerialization.toProtoHashType(parameters.getHashType())).build();
    }
    ...
    private static ProtoParametersSerialization serializeParameters(HmacPrfParameters parameters) throws GeneralSecurityException {
        return ProtoParametersSerialization.create(KeyTemplate.newBuilder().setTypeUrl(TYPE_URL).setValue(HmacPrfKeyFormat.newBuilder().setParams(HmacPrfProtoSerialization.getProtoParams(parameters)).setKeySize(parameters.getKeySizeBytes()).build().toByteString()).setOutputPrefixType(OutputPrefixType.RAW).build());
    }
    ...
    private static ProtoKeySerialization serializeKey(HmacPrfKey key, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        return ProtoKeySerialization.create(TYPE_URL, com.google.crypto.tink.proto.HmacPrfKey.newBuilder().setParams(HmacPrfProtoSerialization.getProtoParams(key.getParameters())).setKeyValue(ByteString.copyFrom(key.getKeyBytes().toByteArray(SecretKeyAccess.requireAccess(access)))).build().toByteString(), KeyData.KeyMaterialType.SYMMETRIC, OutputPrefixType.RAW, key.getIdRequirementOrNull());
    }
    ...
    private static HmacPrfParameters parseParameters(ProtoParametersSerialization serialization) throws GeneralSecurityException {
        ...
        return HmacPrfParameters.builder().setKeySizeBytes(format.getKeySize()).setHashType(HmacPrfProtoSerialization.toHashType(format.getParams().getHash())).build();
    }
    ...
    private static HmacPrfKey parseKey(ProtoKeySerialization serialization, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
        ...
        try {
            ...
            HmacPrfParameters parameters = HmacPrfParameters.builder().setKeySizeBytes(protoKey.getKeyValue().size()).setHashType(HmacPrfProtoSerialization.toHashType(protoKey.getParams().getHash())).build();
    ...
    public static void register() throws GeneralSecurityException {
        HmacPrfProtoSerialization.register(MutableSerializationRegistry.globalInstance());
    }
