...
public final class Field25519 {
    ...
    private static final int[] expandStart = new int[]{0, 3, 6, 9, 12, 16, 19, 22, 25, 28};
    private static final int[] expandShift = new int[]{0, 2, 3, 5, 6, 0, 1, 3, 4, 6};
    ...
    static void sum(long[] output, long[] in) {
        Field25519.sum(output, output, in);
    }
    ...
    static void sub(long[] output, long[] in) {
        Field25519.sub(output, in, output);
    }
    ...
    static void mult(long[] output, long[] in, long[] in2) {
        long[] t = new long[19];
        Field25519.product(t, in, in2);
        Field25519.reduce(t, output);
    }
    ...
    static long[] expand(byte[] input) {
        ...
        for (int i = 0; i < 10; ++i) {
            output[i] = ((long)(input[expandStart[i]] & 0xFF) | (long)(input[expandStart[i] + 1] & 0xFF) << 8 | (long)(input[expandStart[i] + 2] & 0xFF) << 16 | (long)(input[expandStart[i] + 3] & 0xFF) << 24) >> expandShift[i] & (long)mask[i & 1];
        }
        return output;
    }
    ...
    public static byte[] contract(long[] inputLimbs) {
        ...
        for (j = 0; j < 2; ++j) {
            ...
            input[0] = input[0] - (long)carry2 * 19L;
        }
        ...
        input[0] = input[0] + 19L * (long)carry3;
        int mask = Field25519.gte((int)input[0], 67108845);
        for (i = 1; i < 10; ++i) {
            mask &= Field25519.eq((int)input[i], Field25519.mask[i & 1]);
        }
    ...
    static void inverse(long[] out, long[] z) {
        ...
        Field25519.square(t1, z2);
        Field25519.square(t0, t1);
        Field25519.mult(z9, t0, z);
        Field25519.mult(z11, z9, z2);
