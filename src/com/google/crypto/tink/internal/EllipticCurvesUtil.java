...
public final class EllipticCurvesUtil {
    public static final ECParameterSpec NIST_P256_PARAMS = EllipticCurvesUtil.getNistP256Params();
    public static final ECParameterSpec NIST_P384_PARAMS = EllipticCurvesUtil.getNistP384Params();
    public static final ECParameterSpec NIST_P521_PARAMS = EllipticCurvesUtil.getNistP521Params();
    ...
    public static void checkPointOnCurve(ECPoint point, EllipticCurve ec) throws GeneralSecurityException {
        BigInteger rhs;
        BigInteger p = EllipticCurvesUtil.getModulus(ec);
    ...
    public static boolean isNistEcParameterSpec(ECParameterSpec spec) {
        return EllipticCurvesUtil.isSameEcParameterSpec(spec, NIST_P256_PARAMS) || EllipticCurvesUtil.isSameEcParameterSpec(spec, NIST_P384_PARAMS) || EllipticCurvesUtil.isSameEcParameterSpec(spec, NIST_P521_PARAMS);
    }
    ...
    public static ECPoint multiplyByGenerator(BigInteger x, ECParameterSpec spec) throws GeneralSecurityException {
        ...
        EllipticCurvesUtil.checkPointOnCurve(generator, curve);
        BigInteger a = spec.getCurve().getA();
        BigInteger modulus = EllipticCurvesUtil.getModulus(curve);
        JacobianEcPoint r0 = EllipticCurvesUtil.toJacobianEcPoint(ECPoint.POINT_INFINITY, modulus);
        JacobianEcPoint r1 = EllipticCurvesUtil.toJacobianEcPoint(generator, modulus);
        for (int i = x.bitLength(); i >= 0; --i) {
            if (x.testBit(i)) {
                r0 = EllipticCurvesUtil.addJacobianPoints(r0, r1, a, modulus);
                r1 = EllipticCurvesUtil.doubleJacobianPoint(r1, a, modulus);
                continue;
            }
            r1 = EllipticCurvesUtil.addJacobianPoints(r0, r1, a, modulus);
            r0 = EllipticCurvesUtil.doubleJacobianPoint(r0, a, modulus);
        }
        ECPoint output = r0.toECPoint(modulus);
        EllipticCurvesUtil.checkPointOnCurve(output, curve);
        return output;
    }
    ...
    static JacobianEcPoint doubleJacobianPoint(JacobianEcPoint p, BigInteger a, BigInteger modulus) {
        ...
        BigInteger x3 = t = m.multiply(m).mod(modulus).subtract(s.multiply(TWO)).mod(modulus);
    ...
    static JacobianEcPoint addJacobianPoints(JacobianEcPoint p1, JacobianEcPoint p2, BigInteger a, BigInteger modulus) {
        ...
        if (u1.equals(u2)) {
            ...
            return EllipticCurvesUtil.doubleJacobianPoint(p1, a, modulus);
        }
