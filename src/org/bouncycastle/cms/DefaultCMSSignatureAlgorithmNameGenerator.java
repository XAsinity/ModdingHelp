...
implements CMSSignatureAlgorithmNameGenerator {
    ...
    private void addEntries(ASN1ObjectIdentifier aSN1ObjectIdentifier, String string, String string2) {
        this.addDigestAlg(aSN1ObjectIdentifier, string);
        this.addEncryptionAlg(aSN1ObjectIdentifier, string2);
    }

    private void addSimpleAlg(ASN1ObjectIdentifier aSN1ObjectIdentifier, String string) {
        if (this.simpleAlgs.containsKey(aSN1ObjectIdentifier)) {
            throw new IllegalStateException("object identifier already present in addSimpleAlg");
        }
        this.simpleAlgs.put(aSN1ObjectIdentifier, string);
    }

    private void addDigestAlg(ASN1ObjectIdentifier aSN1ObjectIdentifier, String string) {
        if (this.digestAlgs.containsKey(aSN1ObjectIdentifier)) {
    ...
    private void addEncryptionAlg(ASN1ObjectIdentifier aSN1ObjectIdentifier, String string) {
        if (this.encryptionAlgs.containsKey(aSN1ObjectIdentifier)) {
    ...
    public String getSignatureName(AlgorithmIdentifier algorithmIdentifier, AlgorithmIdentifier algorithmIdentifier2) {
        ASN1ObjectIdentifier aSN1ObjectIdentifier = algorithmIdentifier2.getAlgorithm();
        String string = (String)this.simpleAlgs.get(aSN1ObjectIdentifier);
        if (string != null) {
            return string;
        }
        ...
        String string2 = this.getDigestAlgName(aSN1ObjectIdentifier);
        if (!string2.equals(aSN1ObjectIdentifier.getId())) {
            return string2 + "with" + this.getEncryptionAlgName(aSN1ObjectIdentifier);
        }
        return this.getDigestAlgName(algorithmIdentifier.getAlgorithm()) + "with" + this.getEncryptionAlgName(aSN1ObjectIdentifier);
    }
}
