...
implements KeyAgreeRecipient {
    ...
    public JceKeyAgreeRecipient setUnwrappingProvider(String string) {
        this.unwrappingHelper = new EnvelopedDataHelper(new NamedJcaJceExtHelper(string));
        return this;
    }
    ...
    private SecretKey calculateAgreedWrapKey(AlgorithmIdentifier algorithmIdentifier, AlgorithmIdentifier algorithmIdentifier2, PublicKey publicKey, ASN1OctetString aSN1OctetString, PrivateKey privateKey, KeyMaterialGenerator keyMaterialGenerator) throws CMSException, GeneralSecurityException, IOException {
        ...
        if (CMSUtils.isMQV(algorithmIdentifier.getAlgorithm())) {
            ...
            SubjectPublicKeyInfo subjectPublicKeyInfo = new SubjectPublicKeyInfo(this.getPrivateKeyAlgorithmIdentifier(), mQVuserKeyingMaterial.getEphemeralPublicKey().getPublicKeyData());
        ...
        if (CMSUtils.isEC(algorithmIdentifier.getAlgorithm())) {
            byte[] byArray = aSN1OctetString != null ? keyMaterialGenerator.generateKDFMaterial(algorithmIdentifier2, this.keySizeProvider.getKeySize(algorithmIdentifier2), aSN1OctetString.getOctets()) : keyMaterialGenerator.generateKDFMaterial(algorithmIdentifier2, this.keySizeProvider.getKeySize(algorithmIdentifier2), null);
    ...
    protected Key unwrapSessionKey(ASN1ObjectIdentifier aSN1ObjectIdentifier, SecretKey secretKey, ASN1ObjectIdentifier aSN1ObjectIdentifier2, byte[] byArray) throws CMSException, InvalidKeyException, NoSuchAlgorithmException {
        Cipher cipher = this.unwrappingHelper.createCipher(aSN1ObjectIdentifier);
