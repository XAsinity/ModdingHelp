...
public class JcaContentVerifierProviderBuilder {
        ...
        return new ContentVerifierProvider(){
            ...
            public ContentVerifier get(AlgorithmIdentifier algorithmIdentifier) throws OperatorCreationException {
                ...
                if (algorithmIdentifier.getAlgorithm().equals(MiscObjectIdentifiers.id_alg_composite)) {
                    return this.this$0.createCompositeVerifier(algorithmIdentifier, x509Certificate.getPublicKey());
                }
                try {
                    signature = this.this$0.helper.createSignature(algorithmIdentifier);
                ...
                Signature signature2 = this.this$0.createRawSig(algorithmIdentifier, x509Certificate.getPublicKey());
                if (signature2 != null) {
    ...
    public ContentVerifierProvider build(final PublicKey publicKey) throws OperatorCreationException {
        return new ContentVerifierProvider(){
            ...
            public ContentVerifier get(AlgorithmIdentifier algorithmIdentifier) throws OperatorCreationException {
                if (algorithmIdentifier.getAlgorithm().equals(MiscObjectIdentifiers.id_alg_composite)) {
                    return this.this$0.createCompositeVerifier(algorithmIdentifier, publicKey);
                }
                if (publicKey instanceof CompositePublicKey && ((CompositePublicKey)publicKey).getAlgorithmIdentifier().getAlgorithm().equals(MiscObjectIdentifiers.id_composite_key)) {
                    ...
                    for (int i = 0; i != list.size(); ++i) {
                        try {
                            Signature signature = this.this$0.createSignature(algorithmIdentifier, list.get(i));
                            Signature signature2 = this.this$0.createRawSig(algorithmIdentifier, list.get(i));
                            if (signature2 != null) {
                                return new RawSigVerifier(algorithmIdentifier, signature, signature2);
                            }
                            return new SigVerifier(algorithmIdentifier, signature);
                ...
                Signature signature = this.this$0.createSignature(algorithmIdentifier, publicKey);
                Signature signature3 = this.this$0.createRawSig(algorithmIdentifier, publicKey);
                if (signature3 != null) {
    ...
    implements ContentVerifier {
        ...
        public boolean verify(byte[] byArray) {
            try {
                ...
                boolean bl = false;
                for (int i = 0; i != aSN1Sequence.size(); ++i) {
                    if (this.sigs[i] == null || this.sigs[i].verify(ASN1BitString.getInstance(aSN1Sequence.getObjectAt(i)).getOctets())) continue;
                    bl = true;
                }
                return !bl;
            }
