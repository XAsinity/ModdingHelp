...
implements BlockCipher {
    ...
    public void init(boolean bl, CipherParameters cipherParameters) {
        if (cipherParameters instanceof KeyParameter) {
            if (((KeyParameter)cipherParameters).getKeyLength() > 8) {
                throw new IllegalArgumentException("DES key too long - should be 8 bytes");
            }
            this.forEncryption = bl;
            this.workingKey = this.generateWorkingKey(bl, ((KeyParameter)cipherParameters).getKey());
            CryptoServicesRegistrar.checkConstraints(new DefaultServiceProperties(this.getAlgorithmName(), 56, cipherParameters, Utils.getPurpose(this.forEncryption)));
            return;
        }
