...
implements BlockCipher {
    ...
    public void init(boolean bl, CipherParameters cipherParameters) {
        if (cipherParameters instanceof KeyParameter) {
            ...
            int n = this.workingKey.length * 8;
            switch (n) {
                case 128: 
                case 192: 
                case 256: {
            ...
            CryptoServicesRegistrar.checkConstraints(new DefaultServiceProperties(this.getAlgorithmName(), n, cipherParameters, Utils.getPurpose(bl)));
    ...
    private void setKey(byte[] byArray) {
        ...
        for (n3 = 0; n3 < this.k64Cnt; ++n3) {
            ...
            nArray2[n3] = Pack.littleEndianToInt(byArray, n2 + 4);
    ...
    private void encryptBlock(byte[] byArray, int n, byte[] byArray2, int n2) {
        int n3 = Pack.littleEndianToInt(byArray, n) ^ this.gSubKeys[0];
        int n4 = Pack.littleEndianToInt(byArray, n + 4) ^ this.gSubKeys[1];
        int n5 = Pack.littleEndianToInt(byArray, n + 8) ^ this.gSubKeys[2];
        int n6 = Pack.littleEndianToInt(byArray, n + 12) ^ this.gSubKeys[3];
        int n7 = 8;
        for (int i = 0; i < 16; i += 2) {
            ...
            n6 = Integers.rotateLeft(n6, 1) ^ n8 + 2 * n9 + this.gSubKeys[n7++];
            n8 = this.Fe32_0(n5);
            n9 = this.Fe32_3(n6);
            n3 ^= n8 + n9 + this.gSubKeys[n7++];
            n3 = Integers.rotateRight(n3, 1);
            n4 = Integers.rotateLeft(n4, 1) ^ n8 + 2 * n9 + this.gSubKeys[n7++];
        }
        Pack.intToLittleEndian(n5 ^ this.gSubKeys[4], byArray2, n2);
        Pack.intToLittleEndian(n6 ^ this.gSubKeys[5], byArray2, n2 + 4);
        Pack.intToLittleEndian(n3 ^ this.gSubKeys[6], byArray2, n2 + 8);
        Pack.intToLittleEndian(n4 ^ this.gSubKeys[7], byArray2, n2 + 12);
    }

    private void decryptBlock(byte[] byArray, int n, byte[] byArray2, int n2) {
        int n3 = Pack.littleEndianToInt(byArray, n) ^ this.gSubKeys[4];
        int n4 = Pack.littleEndianToInt(byArray, n + 4) ^ this.gSubKeys[5];
        int n5 = Pack.littleEndianToInt(byArray, n + 8) ^ this.gSubKeys[6];
        int n6 = Pack.littleEndianToInt(byArray, n + 12) ^ this.gSubKeys[7];
        int n7 = 39;
        for (int i = 0; i < 16; i += 2) {
            int n8 = this.Fe32_0(n3);
            int n9 = this.Fe32_3(n4);
            n6 ^= n8 + 2 * n9 + this.gSubKeys[n7--];
            n5 = Integers.rotateLeft(n5, 1) ^ n8 + n9 + this.gSubKeys[n7--];
            n6 = Integers.rotateRight(n6, 1);
            n8 = this.Fe32_0(n5);
            n9 = this.Fe32_3(n6);
            n4 ^= n8 + 2 * n9 + this.gSubKeys[n7--];
            n3 = Integers.rotateLeft(n3, 1) ^ n8 + n9 + this.gSubKeys[n7--];
            n4 = Integers.rotateRight(n4, 1);
        }
        Pack.intToLittleEndian(n5 ^ this.gSubKeys[0], byArray2, n2);
        Pack.intToLittleEndian(n6 ^ this.gSubKeys[1], byArray2, n2 + 4);
        Pack.intToLittleEndian(n3 ^ this.gSubKeys[2], byArray2, n2 + 8);
        Pack.intToLittleEndian(n4 ^ this.gSubKeys[3], byArray2, n2 + 12);
    }
