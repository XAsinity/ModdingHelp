...
implements ExtendedDigest {
    ...
    public Blake2sDigest(int n, CryptoServicePurpose cryptoServicePurpose) {
        if (n < 8 || n > 256 || n % 8 != 0) {
    ...
    public Blake2sDigest(byte[] byArray, int n, byte[] byArray2, byte[] byArray3) {
        this(byArray, n, byArray2, byArray3, CryptoServicePurpose.ANY);
    }

    public Blake2sDigest(byte[] byArray, int n, byte[] byArray2, byte[] byArray3, CryptoServicePurpose cryptoServicePurpose) {
        if (n < 1 || n > 32) {
            throw new IllegalArgumentException("Invalid digest length (required: 1 - 32)");
        }
        this.digestLength = n;
        this.purpose = cryptoServicePurpose;
        CryptoServicesRegistrar.checkConstraints(Utils.getDefaultProperties(this, n * 8, cryptoServicePurpose));
        this.init(byArray2, byArray3, byArray);
    }

    Blake2sDigest(int n, byte[] byArray, byte[] byArray2, byte[] byArray3, long l, CryptoServicePurpose cryptoServicePurpose) {
        this.digestLength = n;
        this.nodeOffset = l;
        this.purpose = cryptoServicePurpose;
        CryptoServicesRegistrar.checkConstraints(Utils.getDefaultProperties(this, n * 8, cryptoServicePurpose));
    ...
    Blake2sDigest(int n, int n2, long l) {
        this(n, n2, l, CryptoServicePurpose.ANY);
    }

    Blake2sDigest(int n, int n2, long l, CryptoServicePurpose cryptoServicePurpose) {
        this.digestLength = n;
        this.nodeOffset = l;
        this.fanout = 0;
        this.depth = 0;
    ...
    public int doFinal(byte[] byArray, int n) {
        if (n > byArray.length - this.digestLength) {
        ...
        int n2 = this.digestLength >>> 2;
        int n3 = this.digestLength & 3;
        Pack.intToLittleEndian(this.chainValue, 0, n2, byArray, n);
        if (n3 > 0) {
            byte[] byArray2 = new byte[4];
            Pack.intToLittleEndian(this.chainValue[n2], byArray2, 0);
            System.arraycopy(byArray2, 0, byArray, n + this.digestLength - n3, n3);
        }
    ...
    private void compress(byte[] byArray, int n) {
        ...
        Pack.littleEndianToInt(byArray, n, nArray);
        for (n2 = 0; n2 < 10; ++n2) {
