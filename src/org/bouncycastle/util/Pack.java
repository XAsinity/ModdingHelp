...
public abstract class Pack {
    ...
    public static long bigEndianToLong(byte[] byArray, int n, int n2) {
        long l = 0L;
        for (int i = 0; i < n2; ++i) {
            l |= ((long)byArray[i + n] & 0xFFL) << (7 - i << 3);
        }
        return l;
    }
    ...
    public static void littleEndianToShort(byte[] byArray, int n, short[] sArray, int n2, int n3) {
        for (int i = 0; i < n3; ++i) {
            sArray[n2 + i] = Pack.littleEndianToShort(byArray, n);
            n += 2;
        }
    }
    ...
    public static int littleEndianToInt_High(byte[] byArray, int n, int n2) {
        return Pack.littleEndianToInt_Low(byArray, n, n2) << (4 - n2 << 3);
    }

    public static int littleEndianToInt_Low(byte[] byArray, int n, int n2) {
        int n3 = byArray[n] & 0xFF;
        int n4 = 0;
        for (int i = 1; i < n2; ++i) {
            n3 |= (byArray[n + i] & 0xFF) << (n4 += 8);
        }
        return n3;
    }
    ...
    public static void shortToLittleEndian(short[] sArray, int n, int n2, byte[] byArray, int n3) {
        for (int i = 0; i < n2; ++i) {
            Pack.shortToLittleEndian(sArray[n + i], byArray, n3);
            n3 += 2;
        }
    }
    ...
    public static long littleEndianToLong(byte[] byArray, int n, int n2) {
        long l = 0L;
        for (int i = 0; i < n2; ++i) {
            l |= ((long)byArray[n + i] & 0xFFL) << (i << 3);
        }
        return l;
    }
    ...
    public static void longToLittleEndian_High(long l, byte[] byArray, int n, int n2) {
        int n3 = 56;
        byArray[n] = (byte)(l >>> n3);
        for (int i = 1; i < n2; ++i) {
            byArray[n + i] = (byte)(l >>> (n3 -= 8));
        }
    }

    public static void longToLittleEndian(long l, byte[] byArray, int n, int n2) {
        for (int i = 0; i < n2; ++i) {
            byArray[n + i] = (byte)(l >>> (i << 3));
        }
    }

    public static long littleEndianToLong_High(byte[] byArray, int n, int n2) {
        return Pack.littleEndianToLong_Low(byArray, n, n2) << (8 - n2 << 3);
    }

    public static long littleEndianToLong_Low(byte[] byArray, int n, int n2) {
