...
class Utils {
    ...
    private static boolean isRaw(byte[] byArray) {
        ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(byArray);
        int n = byteArrayInputStream.read();
        int n2 = Utils.readLen(byteArrayInputStream);
        return n2 != byteArrayInputStream.available();
    }

    static ASN1OctetString parseOctetData(byte[] byArray) {
        if (!Utils.isRaw(byArray) && byArray[0] == 4) {
            return ASN1OctetString.getInstance(byArray);
        }
        return null;
    }

    static ASN1Primitive parseData(byte[] byArray) {
        if (!Utils.isRaw(byArray)) {
            if (byArray[0] == 48) {
                return ASN1Sequence.getInstance(byArray);
            }
            if (byArray[0] == 4) {
                return ASN1OctetString.getInstance(byArray);
            }
            if ((byArray[0] & 0xFF) == 128) {
                return ASN1OctetString.getInstance(ASN1TaggedObject.getInstance(byArray), false);
            }
        }
        return null;
    }
