...
class Ntt {
    ...
    public static short[] ntt(short[] sArray) {
        ...
        for (int i = 128; i >= 2; i >>= 1) {
            ...
            while (n2 < 256) {
                int n3;
                short s = nttZetas[n++];
                for (n3 = n2; n3 < n2 + i; ++n3) {
                    short s2 = Ntt.factorQMulMont(s, sArray2[n3 + i]);
                    sArray2[n3 + i] = (short)(sArray2[n3] - s2);
    ...
    public static short[] invNtt(short[] sArray) {
        ...
        for (int i = 2; i <= 128; i <<= 1) {
            int n3 = 0;
            while (n3 < 256) {
                short s = nttZetasInv[n2++];
                for (n = n3; n < n3 + i; ++n) {
                    short s2 = sArray2[n];
                    sArray2[n] = Reduce.barretReduce((short)(s2 + sArray2[n + i]));
                    sArray2[n + i] = (short)(s2 - sArray2[n + i]);
                    sArray2[n + i] = Ntt.factorQMulMont(s, sArray2[n + i]);
                }
                n3 = n + i;
            }
        }
        for (n = 0; n < 256; ++n) {
            sArray2[n] = Ntt.factorQMulMont(sArray2[n], nttZetasInv[127]);
        }
        return sArray2;
    }
    ...
    public static void baseMult(Poly poly, int n, short s, short s2, short s3, short s4, short s5) {
        short s6 = Ntt.factorQMulMont(s2, s4);
        s6 = Ntt.factorQMulMont(s6, s5);
        s6 = (short)(s6 + Ntt.factorQMulMont(s, s3));
        poly.setCoeffIndex(n, s6);
        short s7 = Ntt.factorQMulMont(s, s4);
        s7 = (short)(s7 + Ntt.factorQMulMont(s2, s3));
        poly.setCoeffIndex(n + 1, s7);
    }
}
