...
extends CipherSpi {
    ...
    SABERCipherSpi(String string) throws NoSuchAlgorithmException {
    ...
    protected void engineInit(int n, Key key, AlgorithmParameterSpec algorithmParameterSpec, SecureRandom secureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
        ...
        if (n == 3) {
            if (!(key instanceof BCSABERPublicKey)) throw new InvalidKeyException("Only a " + this.algorithmName + " public key can be used for wrapping");
            ...
            if (!(key instanceof BCSABERPrivateKey)) throw new InvalidKeyException("Only a " + this.algorithmName + " private key can be used for unwrapping");
    ...
    protected Key engineUnwrap(byte[] byArray, String string, int n) throws InvalidKeyException, NoSuchAlgorithmException {
        ...
        try {
            ...
            byte[] byArray2 = sABERKEMExtractor.extractSecret(Arrays.copyOfRange(byArray, 0, sABERKEMExtractor.getEncapsulationLength()));
            Wrapper wrapper = WrapUtil.getWrapper(this.kemParameterSpec.getKeyAlgorithmName());
            KeyParameter keyParameter = new KeyParameter(byArray2);
            Arrays.clear(byArray2);
            wrapper.init(false, keyParameter);
            byte[] byArray3 = Arrays.copyOfRange(byArray, sABERKEMExtractor.getEncapsulationLength(), byArray.length);
