...
public abstract class ECPoint {
        ...
        ValidityPrecompInfo validityPrecompInfo = (ValidityPrecompInfo)this.getCurve().precompute(this, "bc_validity", new PreCompCallback(){
            ...
            public PreCompInfo precompute(PreCompInfo preCompInfo) {
                ...
                if (!validityPrecompInfo.hasCurveEquationPassed()) {
                    if (!bl && !this.this$0.satisfiesCurveEquation()) {
    ...
    public String toString() {
        ...
        stringBuilder.append(this.getRawXCoord());
        stringBuilder.append(',');
        stringBuilder.append(this.getRawYCoord());
        for (int i = 0; i < this.zs.length; ++i) {
            stringBuilder.append(',');
            stringBuilder.append(this.zs[i]);
        }
        stringBuilder.append(')');
    ...
    public int getEncodedLength(boolean bl) {
        if (this.isInfinity()) {
            return 1;
        }
        if (bl) {
            return 1 + this.getXCoord().getEncodedLength();
        }
        return 1 + this.getXCoord().getEncodedLength() + this.getYCoord().getEncodedLength();
    }

    public void encodeTo(boolean bl, byte[] byArray, int n) {
        if (this.isInfinity()) {
            byArray[n] = 0;
            return;
        }
        ...
        if (bl) {
            byArray[n] = (byte)(eCPoint.getCompressionYTilde() ? 3 : 2);
            eCFieldElement.encodeTo(byArray, n + 1);
            return;
        }
        byArray[n] = 4;
        eCFieldElement.encodeTo(byArray, n + 1);
        eCFieldElement2.encodeTo(byArray, n + 1 + eCFieldElement.getEncodedLength());
    }
