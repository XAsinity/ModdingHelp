...
public abstract class ECCurve {
    ...
    public int getAffinePointEncodingLength(boolean bl) {
        int n = this.getFieldElementEncodingLength();
        return bl ? 1 + n : 1 + n * 2;
    }
    ...
    public ECLookupTable createCacheSafeLookupTable(ECPoint[] eCPointArray, int n, int n2) {
        int n3 = this.getFieldElementEncodingLength();
        byte[] byArray = new byte[n2 * n3 * 2];
        int n4 = 0;
        for (int i = 0; i < n2; ++i) {
            ECPoint eCPoint = eCPointArray[n + i];
            eCPoint.getRawXCoord().encodeTo(byArray, n4);
            eCPoint.getRawYCoord().encodeTo(byArray, n4 += n3);
            n4 += n3;
        }
        return new AbstractECLookupTable(this){
            ...
            private ECPoint createPoint(byte[] byArray3, byte[] byArray2) {
                return this.this$0.createRawPoint(this.this$0.fromBigInteger(new BigInteger(1, byArray3)), this.this$0.fromBigInteger(new BigInteger(1, byArray2)));
            }
        };
    }
    ...
    extends ECCurve {
        ...
        private static FiniteField buildField(int n, int n2, int n3, int n4) {
            int[] nArray;
            if (n > Properties.asInteger("org.bouncycastle.ec.max_f2m_field_size", 1142)) {
                throw new IllegalArgumentException("field size out of range: " + n);
            }
            if ((n3 | n4) == 0) {
    ...
    extends ECCurve {
        ...
        @Override
        public ECFieldElement randomFieldElement(SecureRandom secureRandom) {
            BigInteger bigInteger = this.getQ();
