...
public final class InfoCmp {
    private static final Map<String, Object> CAPS_DEFAULT = new HashMap<String, Object>();
    private static final Map<String, Object> CAPS_LOADED = new HashMap<String, Object>();
    ...
    public static Map<String, Capability> getCapabilitiesByName() {
        LinkedHashMap<String, Capability> capabilities = new LinkedHashMap<String, Capability>();
        try (InputStream is = InfoCmp.class.getResourceAsStream("capabilities.txt");){
            LinkedHashMap<String, Capability> linkedHashMap;
    ...
    public static String getInfoCmp(String terminal) throws IOException, InterruptedException {
        ...
        String caps = InfoCmp.getLoadedInfoCmp(terminal);
        if (caps == null) {
            try {
                p = new ProcessBuilder(OSUtils.INFOCMP_COMMAND, "-x", terminal).start();
                caps = ExecHelper.waitAndCapture(p);
                if (p.exitValue() != 0) {
                    error.addSuppressed(new IOException("Command '" + OSUtils.INFOCMP_COMMAND + " -x " + terminal + "' failed with exit code " + p.exitValue() + " and output '" + caps + "'"));
                    caps = null;
                }
            }
            catch (IOException e) {
        ...
        if (caps == null) {
            try {
                p = new ProcessBuilder(OSUtils.INFOCMP_COMMAND, terminal).start();
                caps = ExecHelper.waitAndCapture(p);
                if (p.exitValue() != 0) {
                    error.addSuppressed(new IOException("Command '" + OSUtils.INFOCMP_COMMAND + " " + terminal + "' failed with exit code " + p.exitValue() + " and output '" + caps + "'"));
                    caps = null;
                }
            }
            catch (IOException e) {
        ...
        if (caps != null) {
            InfoCmp.setLoadedInfoCmp(terminal, caps);
        } else {
            caps = InfoCmp.getDefaultInfoCmp(terminal);
            if (caps == null) {
                throw error;
            }
        }
    ...
    public static void parseInfoCmp(String capabilities, Set<Capability> bools, Map<Capability, Integer> ints, Map<Capability, String> strings) {
        Map<String, Capability> capsByName = InfoCmp.getCapabilitiesByName();
        ...
        for (int i = 1; i < lines.length; ++i) {
            ...
            while (m.find()) {
                ...
                if (cap.contains("#")) {
                    ...
                    int iVal = "0".equals(val) ? 0 : (val.startsWith("0x") ? Integer.parseInt(val.substring(2), 16) : (val.startsWith("0") ? Integer.parseInt(val.substring(1), 8) : Integer.parseInt(val)));
    ...
    static {
        for (String s : Arrays.asList("dumb", "dumb-color", "ansi", "xterm", "xterm-256color", "windows", "windows-256color", "windows-conemu", "windows-vtp", "screen", "screen-256color", "rxvt-unicode", "rxvt-unicode-256color", "rxvt-basic", "rxvt")) {
            InfoCmp.setDefaultInfoCmp(s, () -> InfoCmp.loadDefaultInfoCmp(s));
        }
    }
    ...
    public static enum Capability {
        ...
        public String[] getNames() {
            return (String[])InfoCmp.getCapabilitiesByName().entrySet().stream().filter(e -> e.getValue() == this).map(Map.Entry::getKey).toArray(String[]::new);
        }
