...
import org.jline.terminal.impl.jni.win.NativeWinConsoleWriter;
...
extends AbstractWindowsTerminal<Long> {
    ...
    public static NativeWinSysTerminal createTerminal(TerminalProvider provider, SystemStream systemStream, String name, String type, boolean ansiPassThrough, Charset encoding, boolean nativeSignals, Terminal.SignalHandler signalHandler, boolean paused) throws IOException {
        return NativeWinSysTerminal.createTerminal(provider, systemStream, name, type, ansiPassThrough, encoding, encoding, encoding, encoding, nativeSignals, signalHandler, paused);
    }
    ...
    public static NativeWinSysTerminal createTerminal(TerminalProvider provider, SystemStream systemStream, String name, String type, boolean ansiPassThrough, Charset encoding, Charset stdinEncoding, Charset stdoutEncoding, Charset stderrEncoding, boolean nativeSignals, Terminal.SignalHandler signalHandler, boolean paused) throws IOException {
        ...
        long console = NativeWinSysTerminal.getConsole(systemStream);
        if (Kernel32.GetConsoleMode(console, outMode = new int[1]) == 0) {
            throw new IOException("Failed to get console mode: " + NativeWinSysTerminal.getLastErrorMessage());
        }
        if (ansiPassThrough) {
            type = type != null ? type : (OSUtils.IS_CONEMU ? "windows-conemu" : "windows");
            writer = NativeWinSysTerminal.newConsoleWriter(console);
        } else if (NativeWinSysTerminal.enableVtp(console, outMode[0])) {
            ...
            writer = new WindowsAnsiWriter(new BufferedWriter(NativeWinSysTerminal.newConsoleWriter(console)));
        }
        Charset outputEncoding = systemStream == SystemStream.Error ? stderrEncoding : stdoutEncoding;
        NativeWinSysTerminal terminal = new NativeWinSysTerminal(provider, systemStream, writer, name, type, encoding, stdinEncoding, outputEncoding, nativeSignals, signalHandler, consoleIn, inMode[0], console, outMode[0]);
        if (!paused) {
            terminal.resume();
        }
        return terminal;
    ...
    public Cursor getCursorPosition(IntConsumer discarded) {
        ...
        if (Kernel32.GetConsoleScreenBufferInfo((Long)this.outConsole, info) == 0) {
            throw new IOError(new IOException("Could not get the cursor position: " + NativeWinSysTerminal.getLastErrorMessage()));
        }
    ...
    static String getLastErrorMessage() {
        ...
        return NativeWinSysTerminal.getErrorMessage(errorCode);
    }
