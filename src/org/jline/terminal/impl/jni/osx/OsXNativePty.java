...
extends JniNativePty {
    ...
    public static OsXNativePty current(TerminalProvider provider, SystemStream systemStream) throws IOException {
        try {
            switch (systemStream) {
                case Output: {
                    return new OsXNativePty(provider, SystemStream.Output, -1, null, 0, FileDescriptor.in, 1, FileDescriptor.out, OsXNativePty.ttyname(1));
                }
                case Error: {
                    return new OsXNativePty(provider, SystemStream.Error, -1, null, 0, FileDescriptor.in, 2, FileDescriptor.err, OsXNativePty.ttyname(2));
                }
            }
