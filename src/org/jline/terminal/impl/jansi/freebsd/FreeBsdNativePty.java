...
extends JansiNativePty {
    ...
    public static FreeBsdNativePty current(TerminalProvider provider, SystemStream systemStream) throws IOException {
        try {
            switch (systemStream) {
                case Output: {
                    return new FreeBsdNativePty(provider, systemStream, -1, null, 0, FileDescriptor.in, 1, FileDescriptor.out, FreeBsdNativePty.ttyname());
                }
                case Error: {
                    return new FreeBsdNativePty(provider, systemStream, -1, null, 0, FileDescriptor.in, 2, FileDescriptor.err, FreeBsdNativePty.ttyname());
                }
            }
