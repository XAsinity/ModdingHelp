...
extends JnaNativePty {
    ...
    public static LinuxNativePty current(TerminalProvider provider, SystemStream systemStream) throws IOException {
        switch (systemStream) {
            case Output: {
                return new LinuxNativePty(provider, systemStream, -1, null, 0, FileDescriptor.in, 1, FileDescriptor.out, LinuxNativePty.ttyname(0));
            }
            case Error: {
                return new LinuxNativePty(provider, systemStream, -1, null, 0, FileDescriptor.in, 2, FileDescriptor.err, LinuxNativePty.ttyname(0));
            }
        }
