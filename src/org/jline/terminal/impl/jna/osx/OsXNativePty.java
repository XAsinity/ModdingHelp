...
extends JnaNativePty {
    ...
    public static OsXNativePty current(TerminalProvider provider, SystemStream systemStream) throws IOException {
        switch (systemStream) {
            case Output: {
                return new OsXNativePty(provider, systemStream, -1, null, 0, FileDescriptor.in, 1, FileDescriptor.out, OsXNativePty.ttyname(0));
            }
            case Error: {
                return new OsXNativePty(provider, systemStream, -1, null, 0, FileDescriptor.in, 2, FileDescriptor.err, OsXNativePty.ttyname(0));
            }
        }
