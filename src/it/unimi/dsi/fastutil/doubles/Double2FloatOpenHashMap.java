...
Hash {
    ...
    implements Double2FloatMap.FastEntrySet {
        ...
        public void forEach(Consumer<? super Double2FloatMap.Entry> consumer) {
            if (Double2FloatOpenHashMap.this.containsNullKey) {
                consumer.accept(new MapEntry(Double2FloatOpenHashMap.this.n));
            }
            int pos = Double2FloatOpenHashMap.this.n;
            while (pos-- != 0) {
                if (Double.doubleToLongBits(Double2FloatOpenHashMap.this.key[pos]) == 0L) continue;
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return Double2FloatOpenHashMap.this.size - this.c;
            }
            return Math.min((long)(Double2FloatOpenHashMap.this.size - this.c), (long)((double)Double2FloatOpenHashMap.this.realSize() / (double)Double2FloatOpenHashMap.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
