...
Hash {
    ...
    implements Double2ObjectMap.FastEntrySet<V> {
        ...
        public void forEach(Consumer<? super Double2ObjectMap.Entry<V>> consumer) {
            if (Double2ObjectOpenHashMap.this.containsNullKey) {
                consumer.accept(new MapEntry(Double2ObjectOpenHashMap.this.n));
            }
            int pos = Double2ObjectOpenHashMap.this.n;
            while (pos-- != 0) {
                if (Double.doubleToLongBits(Double2ObjectOpenHashMap.this.key[pos]) == 0L) continue;
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return Double2ObjectOpenHashMap.this.size - this.c;
            }
            return Math.min((long)(Double2ObjectOpenHashMap.this.size - this.c), (long)((double)Double2ObjectOpenHashMap.this.realSize() / (double)Double2ObjectOpenHashMap.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
