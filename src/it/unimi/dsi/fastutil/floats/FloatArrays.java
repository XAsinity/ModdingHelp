...
public final class FloatArrays {
    ...
    public static float[] ensureCapacity(float[] array, int length) {
        return FloatArrays.ensureCapacity(array, length, array.length);
    }
    ...
    public static float[] grow(float[] array, int length) {
        return FloatArrays.grow(array, length, array.length);
    }
    ...
    public static void parallelQuickSort(float[] x, int from, int to, FloatComparator comp) {
        ForkJoinPool pool = FloatArrays.getPool();
    ...
    public static void radixSortIndirect(int[] perm, float[] a, int from, int to, boolean stable) {
        int[] support;
        if (to - from < 1024) {
            FloatArrays.quickSortIndirect(perm, a, from, to);
            if (stable) {
                FloatArrays.stabilize(perm, a, from, to);
            }
            return;
        }
        int maxLevel = 3;
        ...
        while (stackPos > 0) {
            ...
            if (stable) {
                ...
                for (i = 0; i <= lastUsed; ++i) {
                    if (level < 3 && count[i] > 1) {
                        if (count[i] < 1024) {
                            FloatArrays.quickSortIndirect(perm, a, p, p + count[i]);
                            if (stable) {
                                FloatArrays.stabilize(perm, a, p, p + count[i]);
                            }
                        } else {
            ...
            for (int i3 = first; i3 <= end; i3 += count[c]) {
                int t = perm[i3];
                c = FloatArrays.fixFloat(a[t]) >>> shift & 0xFF ^ signMask;
                if (i3 < end) {
                    while (true) {
                        int n4 = c;
                        int n5 = pos[n4] - 1;
                ...
                if (level < 3 && count[c] > 1) {
                    if (count[c] < 1024) {
                        FloatArrays.quickSortIndirect(perm, a, i3, i3 + count[c]);
                        if (stable) {
                            FloatArrays.stabilize(perm, a, i3, i3 + count[c]);
                        }
                    } else {
                        offsetStack[stackPos] = i3;
                        lengthStack[stackPos] = count[c];
    ...
    public static void radixSortIndirect(int[] perm, float[] a, float[] b, int from, int to, boolean stable) {
        ...
        while (stackPos > 0) {
            ...
            for (int i3 = first; i3 <= end; i3 += count[c]) {
                int t = perm[i3];
                c = FloatArrays.fixFloat(k[t]) >>> shift & 0xFF ^ signMask;
                if (i3 < end) {
                    while (true) {
                        int n4 = c;
                        int n5 = pos[n4] - 1;
