...
Hash {
    ...
    implements Float2LongMap.FastEntrySet {
        ...
        public void forEach(Consumer<? super Float2LongMap.Entry> consumer) {
            if (Float2LongOpenHashMap.this.containsNullKey) {
                consumer.accept(new MapEntry(Float2LongOpenHashMap.this.n));
            }
            int pos = Float2LongOpenHashMap.this.n;
            while (pos-- != 0) {
                if (Float.floatToIntBits(Float2LongOpenHashMap.this.key[pos]) == 0) continue;
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return Float2LongOpenHashMap.this.size - this.c;
            }
            return Math.min((long)(Float2LongOpenHashMap.this.size - this.c), (long)((double)Float2LongOpenHashMap.this.realSize() / (double)Float2LongOpenHashMap.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
