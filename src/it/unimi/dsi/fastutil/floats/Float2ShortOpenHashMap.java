...
Hash {
    ...
    implements Float2ShortMap.FastEntrySet {
        ...
        public void forEach(Consumer<? super Float2ShortMap.Entry> consumer) {
            if (Float2ShortOpenHashMap.this.containsNullKey) {
                consumer.accept(new MapEntry(Float2ShortOpenHashMap.this.n));
            }
            int pos = Float2ShortOpenHashMap.this.n;
            while (pos-- != 0) {
                if (Float.floatToIntBits(Float2ShortOpenHashMap.this.key[pos]) == 0) continue;
        ...
        public void fastForEach(Consumer<? super Float2ShortMap.Entry> consumer) {
            ...
            if (Float2ShortOpenHashMap.this.containsNullKey) {
                entry.index = Float2ShortOpenHashMap.this.n;
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        MapSpliterator() {
            this.max = Float2ShortOpenHashMap.this.n;
            this.c = 0;
        ...
        public boolean tryAdvance(ConsumerType action) {
            if (this.mustReturnNull) {
                ...
                ++this.c;
                this.acceptOnIndex(action, Float2ShortOpenHashMap.this.n);
                return true;
            }
            float[] key = Float2ShortOpenHashMap.this.key;
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return Float2ShortOpenHashMap.this.size - this.c;
            }
            return Math.min((long)(Float2ShortOpenHashMap.this.size - this.c), (long)((double)Float2ShortOpenHashMap.this.realSize() / (double)Float2ShortOpenHashMap.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
