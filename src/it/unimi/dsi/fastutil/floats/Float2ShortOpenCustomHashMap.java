...
Hash {
    ...
    implements Float2ShortMap.FastEntrySet {
        ...
        public void forEach(Consumer<? super Float2ShortMap.Entry> consumer) {
            if (Float2ShortOpenCustomHashMap.this.containsNullKey) {
                consumer.accept(new MapEntry(Float2ShortOpenCustomHashMap.this.n));
            }
            int pos = Float2ShortOpenCustomHashMap.this.n;
            while (pos-- != 0) {
                if (Float.floatToIntBits(Float2ShortOpenCustomHashMap.this.key[pos]) == 0) continue;
        ...
        public void fastForEach(Consumer<? super Float2ShortMap.Entry> consumer) {
            ...
            if (Float2ShortOpenCustomHashMap.this.containsNullKey) {
                entry.index = Float2ShortOpenCustomHashMap.this.n;
    ...
    implements ShortSpliterator {
        ...
        final void acceptOnIndex(ShortConsumer action, int index) {
            action.accept(Float2ShortOpenCustomHashMap.this.value[index]);
        }
    ...
    implements FloatSpliterator {
        ...
        final void acceptOnIndex(FloatConsumer action, int index) {
            action.accept(Float2ShortOpenCustomHashMap.this.key[index]);
        }
    ...
    extends MapSpliterator<Consumer<? super Float2ShortMap.Entry>, EntrySpliterator>
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        MapSpliterator() {
            this.max = Float2ShortOpenCustomHashMap.this.n;
            this.c = 0;
            this.mustReturnNull = Float2ShortOpenCustomHashMap.this.containsNullKey;
            this.hasSplit = false;
        }
        ...
        public boolean tryAdvance(ConsumerType action) {
            if (this.mustReturnNull) {
                ...
                ++this.c;
                this.acceptOnIndex(action, Float2ShortOpenCustomHashMap.this.n);
                return true;
            }
            float[] key = Float2ShortOpenCustomHashMap.this.key;
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return Float2ShortOpenCustomHashMap.this.size - this.c;
            }
            return Math.min((long)(Float2ShortOpenCustomHashMap.this.size - this.c), (long)((double)Float2ShortOpenCustomHashMap.this.realSize() / (double)Float2ShortOpenCustomHashMap.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
