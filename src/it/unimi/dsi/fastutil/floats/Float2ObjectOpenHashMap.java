...
Hash {
    ...
    public Float2ObjectOpenHashMap<V> clone() {
        ...
        try {
            c = (Float2ObjectOpenHashMap)super.clone();
        }
    ...
    implements Float2ObjectMap.FastEntrySet<V> {
        ...
        public void forEach(Consumer<? super Float2ObjectMap.Entry<V>> consumer) {
            if (Float2ObjectOpenHashMap.this.containsNullKey) {
                consumer.accept(new MapEntry(Float2ObjectOpenHashMap.this.n));
            }
            int pos = Float2ObjectOpenHashMap.this.n;
            while (pos-- != 0) {
                if (Float.floatToIntBits(Float2ObjectOpenHashMap.this.key[pos]) == 0) continue;
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return Float2ObjectOpenHashMap.this.size - this.c;
            }
            return Math.min((long)(Float2ObjectOpenHashMap.this.size - this.c), (long)((double)Float2ObjectOpenHashMap.this.realSize() / (double)Float2ObjectOpenHashMap.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
