...
Hash {
    ...
    implements Byte2ObjectMap.FastEntrySet<V> {
        ...
        public void forEach(Consumer<? super Byte2ObjectMap.Entry<V>> consumer) {
            if (Byte2ObjectOpenCustomHashMap.this.containsNullKey) {
                consumer.accept(new MapEntry(Byte2ObjectOpenCustomHashMap.this.n));
            }
            int pos = Byte2ObjectOpenCustomHashMap.this.n;
            while (pos-- != 0) {
                if (Byte2ObjectOpenCustomHashMap.this.key[pos] == 0) continue;
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return Byte2ObjectOpenCustomHashMap.this.size - this.c;
            }
            return Math.min((long)(Byte2ObjectOpenCustomHashMap.this.size - this.c), (long)((double)Byte2ObjectOpenCustomHashMap.this.realSize() / (double)Byte2ObjectOpenCustomHashMap.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
