...
Hash {
    ...
    implements Byte2ReferenceMap.FastEntrySet<V> {
        ...
        public void forEach(Consumer<? super Byte2ReferenceMap.Entry<V>> consumer) {
            if (Byte2ReferenceOpenHashMap.this.containsNullKey) {
                consumer.accept(new MapEntry(Byte2ReferenceOpenHashMap.this.n));
            }
            int pos = Byte2ReferenceOpenHashMap.this.n;
            while (pos-- != 0) {
                if (Byte2ReferenceOpenHashMap.this.key[pos] == 0) continue;
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return Byte2ReferenceOpenHashMap.this.size - this.c;
            }
            return Math.min((long)(Byte2ReferenceOpenHashMap.this.size - this.c), (long)((double)Byte2ReferenceOpenHashMap.this.realSize() / (double)Byte2ReferenceOpenHashMap.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
