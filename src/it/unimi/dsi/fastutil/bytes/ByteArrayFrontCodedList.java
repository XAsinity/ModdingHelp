...
RandomAccess {
    ...
    static int readInt(byte[][] a, long pos) {
        ...
        byte b1 = BigArrays.get(a, pos + 1L);
        if (b1 >= 0) {
            return -b0 - 1 << 7 | b1;
        }
        byte b2 = BigArrays.get(a, pos + 2L);
        if (b2 >= 0) {
            return -b0 - 1 << 14 | -b1 - 1 << 7 | b2;
        }
        byte b3 = BigArrays.get(a, pos + 3L);
        if (b3 >= 0) {
            return -b0 - 1 << 21 | -b1 - 1 << 14 | -b2 - 1 << 7 | b3;
        }
        return -b0 - 1 << 28 | -b1 - 1 << 21 | -b2 - 1 << 14 | -b3 - 1 << 7 | BigArrays.get(a, pos + 4L);
    }
    ...
    static int writeInt(byte[][] a, int length, long pos) {
        ...
        BigArrays.set(a, pos + (long)count - 1L, (byte)(length & 0x7F));
        int i = count - 1;
        while (i-- != 0) {
            BigArrays.set(a, pos + (long)i, (byte)(-((length >>>= 7) & 0x7F) - 1));
        }
        return count;
    }
        ...
        return new ObjectListIterator<byte[]>(){
            ...
            public byte[] next() {
                ...
                if (this.i % ByteArrayFrontCodedList.this.ratio == 0) {
                    ...
                    BigArrays.copyFromBig(ByteArrayFrontCodedList.this.array, this.pos + (long)ByteArrayFrontCodedList.count(length), this.s, 0, length);
                    ...
                    BigArrays.copyFromBig(ByteArrayFrontCodedList.this.array, this.pos + (long)ByteArrayFrontCodedList.count(length) + (long)ByteArrayFrontCodedList.count(common), this.s, common, length);
