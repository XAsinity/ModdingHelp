...
Hash {
    ...
    implements Byte2ShortMap.FastEntrySet {
        ...
        public void forEach(Consumer<? super Byte2ShortMap.Entry> consumer) {
            if (Byte2ShortOpenCustomHashMap.this.containsNullKey) {
                consumer.accept(new MapEntry(Byte2ShortOpenCustomHashMap.this.n));
            }
            int pos = Byte2ShortOpenCustomHashMap.this.n;
            while (pos-- != 0) {
                if (Byte2ShortOpenCustomHashMap.this.key[pos] == 0) continue;
    ...
    private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return Byte2ShortOpenCustomHashMap.this.size - this.c;
            }
            return Math.min((long)(Byte2ShortOpenCustomHashMap.this.size - this.c), (long)((double)Byte2ShortOpenCustomHashMap.this.realSize() / (double)Byte2ShortOpenCustomHashMap.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
