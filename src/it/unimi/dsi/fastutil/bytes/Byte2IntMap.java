...
Map<Byte, Integer> {
    ...
    @Override
    default public void forEach(BiConsumer<? super Byte, ? super Integer> consumer) {
        ObjectSet<Entry> entrySet = this.byte2IntEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getByteKey(), entry.getIntValue());
    ...
    default public int computeIfAbsent(byte key, Byte2IntFunction mappingFunction) {
        Objects.requireNonNull(mappingFunction);
        int v = this.get(key);
        int drv = this.defaultReturnValue();
        if (v != drv || this.containsKey(key)) {
    ...
    default public int mergeInt(byte key, int value, java.util.function.IntBinaryOperator remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        int oldValue = this.get(key);
        int drv = this.defaultReturnValue();
    ...
    default public int mergeInt(byte key, int value, IntBinaryOperator remappingFunction) {
