...
Map<Character, Boolean> {
    ...
    @Override
    default public void forEach(BiConsumer<? super Character, ? super Boolean> consumer) {
        ObjectSet<Entry> entrySet = this.char2BooleanEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(Character.valueOf(entry.getCharKey()), entry.getBooleanValue());
    ...
    default public boolean computeIfAbsent(char key, Char2BooleanFunction mappingFunction) {
        Objects.requireNonNull(mappingFunction);
        boolean v = this.get(key);
        boolean drv = this.defaultReturnValue();
        if (v != drv || this.containsKey(key)) {
