...
Map<Integer, Float> {
    ...
    @Override
    default public void forEach(BiConsumer<? super Integer, ? super Float> consumer) {
        ObjectSet<Entry> entrySet = this.int2FloatEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getIntKey(), Float.valueOf(entry.getFloatValue()));
    ...
    default public float computeIfAbsent(int key, Int2FloatFunction mappingFunction) {
        Objects.requireNonNull(mappingFunction);
        float v = this.get(key);
        float drv = this.defaultReturnValue();
        if (v != drv || this.containsKey(key)) {
    ...
    default public float mergeFloat(int key, float value, FloatBinaryOperator remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        float oldValue = this.get(key);
        float drv = this.defaultReturnValue();
    ...
    default public float mergeFloat(int key, float value, DoubleBinaryOperator remappingFunction) {
