...
public final class IntArrays {
    ...
    public static void radixSortIndirect(int[] perm, int[] a, int from, int to, boolean stable) {
        int[] support;
        if (to - from < 1024) {
            IntArrays.quickSortIndirect(perm, a, from, to);
            if (stable) {
                IntArrays.stabilize(perm, a, from, to);
            }
            return;
        }
        int maxLevel = 3;
        ...
        while (stackPos > 0) {
            ...
            if (stable) {
                ...
                for (i = 0; i <= lastUsed; ++i) {
                    if (level < 3 && count[i] > 1) {
                        if (count[i] < 1024) {
                            IntArrays.quickSortIndirect(perm, a, p, p + count[i]);
                            if (stable) {
                                IntArrays.stabilize(perm, a, p, p + count[i]);
                            }
                        } else {
            ...
            for (int i3 = first; i3 <= end; i3 += count[c]) {
                ...
                if (level < 3 && count[c] > 1) {
                    if (count[c] < 1024) {
                        IntArrays.quickSortIndirect(perm, a, i3, i3 + count[c]);
                        if (stable) {
                            IntArrays.stabilize(perm, a, i3, i3 + count[c]);
                        }
                    } else {
                        offsetStack[stackPos] = i3;
                        lengthStack[stackPos] = count[c];
