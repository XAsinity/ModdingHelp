...
public final class LongArrays {
    ...
    public static long[] ensureCapacity(long[] array, int length) {
        return LongArrays.ensureCapacity(array, length, array.length);
    }

    public static long[] ensureCapacity(long[] array, int length, int preserve) {
        return length > array.length ? LongArrays.forceCapacity(array, length, preserve) : array;
    }

    public static long[] grow(long[] array, int length) {
        return LongArrays.grow(array, length, array.length);
    }
    ...
    public static void parallelQuickSort(long[] x, int from, int to, LongComparator comp) {
        ForkJoinPool pool = LongArrays.getPool();
    ...
    public static void mergeSort(long[] a, int from, int to) {
        LongArrays.mergeSort(a, from, to, (long[])null);
    }
    ...
    public static void mergeSort(long[] a, int from, int to, LongComparator comp) {
        LongArrays.mergeSort(a, from, to, comp, null);
    }
    ...
    public static void stableSort(long[] a, int from, int to) {
        LongArrays.unstableSort(a, from, to);
    }

    public static void stableSort(long[] a) {
        LongArrays.stableSort(a, 0, a.length);
    }
    ...
    public static void radixSortIndirect(int[] perm, long[] a, int from, int to, boolean stable) {
        int[] support;
        if (to - from < 1024) {
            LongArrays.quickSortIndirect(perm, a, from, to);
            if (stable) {
                LongArrays.stabilize(perm, a, from, to);
            }
            return;
        }
        int maxLevel = 7;
        ...
        while (stackPos > 0) {
            ...
            int i2 = first + length;
            while (i2-- != first) {
                int n = (int)(a[perm[i2]] >>> shift & 0xFFL ^ (long)signMask);
                count[n] = count[n] + 1;
            }
            int lastUsed = -1;
            ...
            if (stable) {
                i = first + length;
                while (i-- != first) {
                    int n2 = (int)(a[perm[i]] >>> shift & 0xFFL ^ (long)signMask);
                    int n3 = pos[n2] - 1;
                    pos[n2] = n3;
                    support[n3] = perm[i];
                }
                ...
                for (i = 0; i <= lastUsed; ++i) {
                    if (level < 7 && count[i] > 1) {
                        if (count[i] < 1024) {
                            LongArrays.quickSortIndirect(perm, a, p, p + count[i]);
                            if (stable) {
                                LongArrays.stabilize(perm, a, p, p + count[i]);
                            }
                        } else {
            ...
            for (int i3 = first; i3 <= end; i3 += count[c]) {
                int t = perm[i3];
                c = (int)(a[t] >>> shift & 0xFFL ^ (long)signMask);
                if (i3 < end) {
                    while (true) {
                        int n4 = c;
                        int n5 = pos[n4] - 1;
                ...
                if (level < 7 && count[c] > 1) {
                    if (count[c] < 1024) {
                        LongArrays.quickSortIndirect(perm, a, i3, i3 + count[c]);
                        if (stable) {
                            LongArrays.stabilize(perm, a, i3, i3 + count[c]);
                        }
                    } else {
                        offsetStack[stackPos] = i3;
                        lengthStack[stackPos] = count[c];
        ...
        while (j-- != 0) {
            executorCompletionService.submit(() -> {
                ...
                while (true) {
                    ...
                    int i3 = first + length;
                    while (i3-- != first) {
                        int n = (int)(a[perm[i3]] >>> shift & 0xFFL ^ (long)signMask);
                        count[n] = count[n] + 1;
                    }
                    int lastUsed = -1;
                    int p = first;
                    ...
                    if (stable) {
                        i = first + length;
                        while (i-- != first) {
                            int n = (int)(a[perm[i]] >>> shift & 0xFFL ^ (long)signMask);
                            int n2 = pos[n] - 1;
                            pos[n] = n2;
                            support[n2] = perm[i];
                        }
    ...
    public static void radixSort(long[] a, long[] b, int from, int to) {
        ...
        while (stackPos > 0) {
            ...
            int i = first + length;
            while (i-- != first) {
                int n = (int)(k[i] >>> shift & 0xFFL ^ (long)signMask);
                count[n] = count[n] + 1;
            }
            int lastUsed = -1;
            int p = first;
            ...
            for (int i3 = first; i3 <= end; i3 += count[c]) {
                long t = a[i3];
                long u = b[i3];
                c = (int)(k[i3] >>> shift & 0xFFL ^ (long)signMask);
                if (i3 < end) {
                    while (true) {
                        int n = c;
                        int n2 = pos[n] - 1;
                        ...
                        int d = n2;
                        if (n2 <= i3) break;
                        c = (int)(k[d] >>> shift & 0xFFL ^ (long)signMask);
                        long z = t;
                        t = a[d];
                        a[d] = z;
                        z = u;
    ...
    public static void radixSortIndirect(int[] perm, long[] a, long[] b, int from, int to, boolean stable) {
        ...
        while (stackPos > 0) {
            ...
            int i2 = first + length;
            while (i2-- != first) {
                int n = (int)(k[perm[i2]] >>> shift & 0xFFL ^ (long)signMask);
                count[n] = count[n] + 1;
            }
            int lastUsed = -1;
            int n = p = stable ? 0 : first;
            ...
            if (stable) {
                i = first + length;
                while (i-- != first) {
                    int n2 = (int)(k[perm[i]] >>> shift & 0xFFL ^ (long)signMask);
                    int n3 = pos[n2] - 1;
                    pos[n2] = n3;
                    support[n3] = perm[i];
                }
            ...
            for (int i3 = first; i3 <= end; i3 += count[c]) {
                int t = perm[i3];
                c = (int)(k[t] >>> shift & 0xFFL ^ (long)signMask);
                if (i3 < end) {
                    while (true) {
                        int n4 = c;
                        int n5 = pos[n4] - 1;
