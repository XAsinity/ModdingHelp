...
Hash {
    ...
    implements ShortIterator {
        ...
        public void forEachRemaining(ShortConsumer action) {
            ...
            if (this.mustReturnNull) {
                this.mustReturnNull = false;
                this.last = ShortOpenCustomHashSet.this.n;
                action.accept(key[ShortOpenCustomHashSet.this.n]);
                --this.c;
            }
            while (this.c != 0) {
                if (--this.pos < 0) {
    ...
    implements ShortSpliterator {
        ...
        SetSpliterator() {
            this.max = ShortOpenCustomHashSet.this.n;
            this.c = 0;
            this.mustReturnNull = ShortOpenCustomHashSet.this.containsNull;
            this.hasSplit = false;
        }
        ...
        public boolean tryAdvance(ShortConsumer action) {
            if (this.mustReturnNull) {
                this.mustReturnNull = false;
                ++this.c;
                action.accept(ShortOpenCustomHashSet.this.key[ShortOpenCustomHashSet.this.n]);
                return true;
            }
            short[] key = ShortOpenCustomHashSet.this.key;
        ...
        public long estimateSize() {
            if (!this.hasSplit) {
                return ShortOpenCustomHashSet.this.size - this.c;
            }
            return Math.min((long)(ShortOpenCustomHashSet.this.size - this.c), (long)((double)ShortOpenCustomHashSet.this.realSize() / (double)ShortOpenCustomHashSet.this.n * (double)(this.max - this.pos)) + (long)(this.mustReturnNull ? 1 : 0));
        }
