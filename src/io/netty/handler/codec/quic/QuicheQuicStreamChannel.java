...
import io.netty.handler.codec.quic.QuicheQuicStreamChannelConfig;
...
implements QuicStreamChannel {
    ...
    boolean writable(long capacity) {
        assert (this.eventLoop().inEventLoop());
        if (capacity < 0L) {
            if (capacity != (long)Quiche.QUICHE_ERR_DONE) {
                if (!this.queue.isEmpty()) {
                    if (capacity == (long)Quiche.QUICHE_ERR_STREAM_STOPPED) {
                ...
                } else if (capacity == (long)Quiche.QUICHE_ERR_STREAM_STOPPED) {
                    return false;
                }
                this.finSent = true;
                this.unsafe().close(this.unsafe().voidPromise());
    ...
    implements Channel.Unsafe {
        ...
        void close(@Nullable ClosedChannelException writeFailCause, ChannelPromise promise) {
            ...
            if (!QuicheQuicStreamChannel.this.active || QuicheQuicStreamChannel.this.closePromise.isDone()) {
                ...
                QuicheQuicStreamChannel.this.closePromise.addListener((GenericFutureListener)new PromiseNotifier(promise));
                return;
            }
                    ...
                    try {
                        while (!QuicheQuicStreamChannel.this.finReceived && continueReading) {
                            ...
                            switch (result) {
                                ...
                                default: {
                                    throw new Error("Unexpected StreamRecvResult: " + (Object)((Object)result));
                                }
                            }
                        ...
                        if (!QuicheQuicStreamChannel.this.finReceived) continue;
                        QuicheQuicStreamChannel.this.readable = false;
                        this.closeOnRead(pipeline, readFrames);
                    }
                    catch (Throwable cause) {
