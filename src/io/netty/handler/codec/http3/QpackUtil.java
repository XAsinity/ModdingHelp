...
final class QpackUtil {
    private static final QpackException PREFIXED_INTEGER_TOO_LONG = QpackException.newStatic(QpackDecoder.class, "toIntOrThrow(...)", "QPACK - invalid prefixed integer");
    ...
    static final long MAX_HEADER_TABLE_SIZE = 0xFFFFFFFFL;

    static void encodePrefixedInteger(ByteBuf out, byte mask, int prefixLength, long toEncode) {
        ObjectUtil.checkInRange(toEncode, 0L, 0xFFFFFFFFL, "toEncode");
        int nbits = (1 << prefixLength) - 1;
        if (toEncode < (long)nbits) {
            out.writeByte((byte)((long)mask | toEncode));
        } else {
            long remainder;
            out.writeByte((byte)(mask | nbits));
            for (remainder = toEncode - (long)nbits; remainder > 128L; remainder /= 128L) {
                byte next = (byte)(remainder % 128L | 0x80L);
                out.writeByte(next);
            }
    ...
    static int decodePrefixedIntegerAsInt(ByteBuf in, int prefixLength) throws QpackException {
        return QpackUtil.toIntOrThrow(QpackUtil.decodePrefixedInteger(in, prefixLength));
    }

    static int toIntOrThrow(long aLong) throws QpackException {
        if ((long)((int)aLong) != aLong) {
    ...
    static long decodePrefixedInteger(ByteBuf in, int prefixLength) {
        ...
        if (readerIndex == (writerIndex = in.writerIndex())) {
            return -1L;
        }
        int nbits = (1 << prefixLength) - 1;
    ...
    static long maxEntries(long maxTableCapacity) {
        return Math.floorDiv(maxTableCapacity, 32L);
    }
