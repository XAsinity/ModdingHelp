...
public final class HttpUtil {
    ...
    static void validateRequestLineTokens(HttpMethod method, String uri) {
        if (method.getClass() != HttpMethod.class && !HttpUtil.isEncodingSafeStartLineToken(method.asciiName())) {
            throw new IllegalArgumentException("The HTTP method name contain illegal characters: " + method.asciiName());
        }
        if (!HttpUtil.isEncodingSafeStartLineToken(uri)) {
    ...
    static int validateToken(CharSequence token) {
        ...
        return HttpUtil.validateCharSequenceToken(token);
    }
    ...
    private static int validateAsciiStringToken(AsciiString token) {
        ...
        for (int i = token.arrayOffset(); i < len; ++i) {
            if (HttpUtil.isValidTokenChar(array[i])) continue;
    ...
    private static int validateCharSequenceToken(CharSequence token) {
        ...
        for (int i = 0; i < len; ++i) {
            ...
            if (HttpUtil.isValidTokenChar(value)) continue;
            return i;
        }
        return -1;
    }
