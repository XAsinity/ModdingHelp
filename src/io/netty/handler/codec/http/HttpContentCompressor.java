...
extends HttpContentEncoder {
    ...
    private static CompressionOptions[] defaultCompressionOptions(GzipOptions gzipOptions, DeflateOptions deflateOptions) {
        ArrayList<CompressionOptions> options = new ArrayList<CompressionOptions>(5);
    ...
    protected HttpContentEncoder.Result beginEncode(HttpResponse httpResponse, String acceptEncoding) throws Exception {
        ...
        return new HttpContentEncoder.Result(targetContentEncoding, EmbeddedChannel.builder().channelId(channel.id()).hasDisconnect(channel.metadata().hasDisconnect()).config(channel.config()).handlers((ChannelHandler)encoderFactory.createEncoder()).build());
    }
