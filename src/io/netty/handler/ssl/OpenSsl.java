...
public final class OpenSsl {
    ...
    static String[] defaultProtocols(boolean isClient) {
        ...
        for (String proto : defaultProtocols) {
            if (!OpenSsl.isProtocolSupported(proto)) continue;
            protocols.add(proto);
        }
    ...
    static boolean isProtocolSupported(String protocol) {
        int bit = OpenSsl.getProtocolBit(protocol);
    ...
    static List<String> unpackSupportedProtocols() {
        ArrayList<String> protocols = new ArrayList<String>(7);
        ...
        if (cause == null) {
            ...
            try {
                ...
                while (defaultGroupsIter.hasNext()) {
                    if (SSLContext.setCurvesList((long)sslCtx, (String[])new String[]{(String)defaultGroupsIter.next()})) continue;
                    ...
                    try {
                        ...
                        try {
                            ...
                            keyBio = LeakPresenceDetector.staticInitializer(() -> {
                                try {
                                    return ReferenceCountedOpenSslContext.toBIO((ByteBufAllocator)UnpooledByteBufAllocator.DEFAULT, privateKey.retain());
                                }
                                catch (Exception e) {
                                    PlatformDependent.throwException(e);
                                    throw new AssertionError((Object)e);
            ...
            for (String cipher : AVAILABLE_OPENSSL_CIPHER_SUITES) {
                if (!SslUtils.isTLSv13Cipher(cipher)) {
                    String sslConversion;
                    String tlsConversion = CipherSuiteConverter.toJava(cipher, "TLS");
                    if (tlsConversion != null) {
                        availableJavaCipherSuites.add(tlsConversion);
                    }
                    if ((sslConversion = CipherSuiteConverter.toJava(cipher, "SSL")) == null) continue;
