...
extends ByteBuf {
    ...
    public String readString(int length, Charset charset) {
        String string = this.toString(this.readerIndex, length, charset);
        this.readerIndex += length;
        return string;
    }
    ...
    private static void checkRangeBoundsTrustedCapacity(String indexName, int index, int fieldLength, int capacity) {
        if (AbstractByteBuf.isOutOfBoundsTrustedCapacity(index, fieldLength, capacity)) {
            AbstractByteBuf.rangeBoundsCheckFailed(indexName, index, fieldLength, capacity);
        }
    }
    ...
    final void checkIndex0(int index, int fieldLength) {
        if (checkBounds) {
            AbstractByteBuf.checkRangeBoundsTrustedCapacity("index", index, fieldLength, this.capacity());
        }
    }
