...
import io.netty.buffer.ReadOnlyAbstractByteBuf;
...
public final class Unpooled {
    ...
    public static ByteBuf unmodifiableBuffer(ByteBuf buffer) {
        ...
        if (endianness == BIG_ENDIAN) {
            return Unpooled.newReadyOnlyBuffer(buffer);
        }
        return Unpooled.newReadyOnlyBuffer(buffer.order(BIG_ENDIAN)).order(LITTLE_ENDIAN);
    }

    private static ReadOnlyByteBuf newReadyOnlyBuffer(ByteBuf buffer) {
        return buffer instanceof AbstractByteBuf ? new ReadOnlyAbstractByteBuf((AbstractByteBuf)buffer) : new ReadOnlyByteBuf(buffer);
    }
