...
implements ChannelOutboundBuffer.MessageProcessor {
    ...
    public IovArray(int numEntries) {
        ...
        ByteBuf byteBuf = PlatformDependent.hasUnsafe() ? bbuf : (this.memory = bbuf.order(PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? ByteOrder.BIG_ENDIAN : ByteOrder.LITTLE_ENDIAN));
        if (this.memory.hasMemoryAddress()) {
            ...
            this.memoryAddress = Buffer.memoryAddress(byteBuffer) + (long)byteBuffer.position();
        }
