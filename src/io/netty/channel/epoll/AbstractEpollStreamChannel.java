...
implements DuplexChannel {
    ...
    private void failSpliceIfClosed(ChannelPromise promise) {
        ...
        if (!this.isOpen() && !promise.isDone() && promise.tryFailure(ex = new ClosedChannelException())) {
        ...
        public boolean spliceIn(RecvByteBufAllocator.Handle handle) {
            ...
            try {
                ...
                if ((splicedIn = this.spliceIn(pipeOut, handle)) > 0) {
                    ...
                    ChannelFuture splicePromise = this.len == 0 ? this.promise : this.ch.newPromise().addListener((GenericFutureListener)this);
                    boolean autoRead = AbstractEpollStreamChannel.this.config().isAutoRead();
                    this.ch.unsafe().write(new SpliceOutTask(this.ch, splicedIn, autoRead), (ChannelPromise)splicePromise);
    ...
    extends AbstractEpollChannel.AbstractEpollUnsafe {
        ...
        private void handleReadException(ChannelPipeline pipeline, ByteBuf byteBuf, Throwable cause, boolean allDataRead, EpollRecvByteAllocatorHandle allocHandle) {
            ...
            if (allDataRead || cause instanceof OutOfMemoryError || cause instanceof LeakPresenceDetector.AllocationProhibitedException || cause instanceof IOException) {
